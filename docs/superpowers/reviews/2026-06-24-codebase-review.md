...
`analysis`, `csharp`, `aef`, `cli`, `web`). Responsibilities are clear; clippy
...
|---|---|---|
| `LoopIf.aef` | `loop { let loc0 = 0; if loc0 < 3 { loc0 += 1; } }` | `loc0_0 = 0;` / `if (loc0_0) {` / `t_4 = (0 + 1);` |
| `MultiMethod.aef` | `fn main(){return 1;}` + `fn helper(){return 1+1;}` + contract envelope | `// return/throw/abort at BlockId(0)` |
