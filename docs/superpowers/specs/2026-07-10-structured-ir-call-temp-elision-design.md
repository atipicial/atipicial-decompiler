...
Substituting calls through the optimizer could eliminate more temps, but it would require side-effect-aware ordering rules throughout expression rewriting. In particular, moving a Atipicial call into another call's arguments can change behavior because Atipicial arguments are evaluated right-to-left while C# evaluates them left-to-right. This is broader than the requested cleanup.
...
- Changing Atipicial VM argument order or C# evaluation semantics.
