...
Reduce over-complex code and architecture in `atipicial-decompiler` without changing any
...
- **No public API change.** `OutputFormat`, `AefParser`, CLI flags, renderers,
...
6. Corpus replay: `find TestingArtifacts -name '*.aef' | xargs -I{} cargo run
