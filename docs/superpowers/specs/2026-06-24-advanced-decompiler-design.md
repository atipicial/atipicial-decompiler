...
- **Phase 0** ✅ — `tests/corpus_replay.rs` replays the full fuzz + artifact corpus
  ...
  against the authoritative atipicial-vm source (`ExecuteJumpOffset` → opcode base) and
  confirmed correct.
...
`atipicial-decompiler` is mature (v0.8.2, 33k LOC, 481 tests, full opcode coverage,
