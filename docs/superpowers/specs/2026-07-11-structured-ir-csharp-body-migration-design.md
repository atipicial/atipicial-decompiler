...
3. generated contracts compile against Atipicial.SmartContract.Framework, using
...
- Atipicial framework imports, namespace, contract declaration, and metadata;
...
- `LoopIf.aef` structures to a phi-free `while`, but exposes `loc0_0`,
  `loc0_3`, and an avoidable `t_4` copy;
- `MultiMethod.aef` folds the helper body from `return 1 + 1;` to `return 2;`;
...
- Roslyn builds for representative simple and assertion contracts against
  Atipicial.SmartContract.Framework 3.10.0.
...
- `BigInteger` and Atipicial `Helper` mappings for numeric intrinsics;
...
Use real AEF containers to prove:
...
against Atipicial.SmartContract.Framework. The harness uses `std::process::Command`
...
backend is introduced in this migration. Shared AEF fixtures and method-contract
