...
a single comment (e.g. `MultiMethod.aef` → `// return/throw/abort at
...
`self.aef` / `self.manifest`. Long-term, the pipeline can move to producing
...
then the per-method bodies. The renderer reads `self.aef` (name + hash) and
