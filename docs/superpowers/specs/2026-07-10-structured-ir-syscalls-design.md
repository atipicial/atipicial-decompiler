...
Make the structured SSA/IR path preserve Atipicial syscalls as readable call expressions with exact known arity and return behavior, instead of consuming their stack effects and emitting `?` or no statement.
...
- pop exactly `param_count` values top-first, which is Atipicial's declaration order;
