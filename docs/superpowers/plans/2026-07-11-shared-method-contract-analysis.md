...
**Tech Stack:** Rust 2021, serde/serde_json, Atipicial VM CFG/SSA/typed IR, JavaScript ES modules, Node test runner, TypeScript declarations, JSON Schema.
...
Use the existing AEF/disassembly helpers and the critical script:
...
Change `render_envelope` to accept `&MethodContracts`. Remove `build_inferred_method_contracts`, `infer_method_returns_value`, and its manifest-only return map. Build internal SSA `CallContract`s from `MethodContracts::get(offset)` and use `return_behavior.returns_value()`. Keep method-token contracts from AEF/call-graph metadata and indirect/unresolved calls opaque.
...
Reuse `buildAefFromScript` and the same fixtures as Rust. For the critical fixture, assert the public stable list is exactly offsets 0 and 7, excluding the presentation-only NOP group at offset 5:
