...
**Tech Stack:** Rust, `cfg`, `cfg::ssa`, `ir`, `analysis::methods::MethodTable`, `aef`, `manifest`. TDD; full suite + clippy `-D warnings` + fmt + `--no-default-features` as the fence.
...
pub(crate) fn render_envelope(
    aef: &crate::aef::AefFile,
    manifest: Option<&ContractManifest>,
    methods: &[MethodView],
) -> String {
    let mut out = String::new();
    write_contract_header(&mut out, aef, manifest);
    for view in methods {
...
fn ir_pipeline_renders_per_method_bodies_inside_envelope_for_multimethod() {
    let root = repo_root();
    let aef = fs::read(root.join("TestingArtifacts/edgecases/multi/MultiMethod.aef")).unwrap();
...
Run:
```bash
cargo run --quiet -- decompile TestingArtifacts/edgecases/multi/MultiMethod.aef --format ir
```
...
- `MultiMethod.aef --format ir` renders `contract MultiMethod { ... fn main(){...} fn helper(){...} }`.
- `LoopIf.aef --format ir` renders `contract LoopIf { ... fn ...() { while(...) {...} } }`.
