...
**Tech Stack:** Rust, existing Atipicial VM CFG/SSA/IR modules, `ContractManifest`, `CallGraph`, built-in unit and integration tests. No new dependencies.
...
Build a real AEF method containing `INITSLOT 0,2; LDARG1; RET` and a manifest declaring `choose(from: Hash160, amount: Integer) -> Integer`. Assert that structured IR contains:
...
Assert a known value call pops arguments in Atipicial calling order and pushes only its result. Assert a known void call emits a side-effect statement, pushes no result, and leaves a later return value intact.
...
Use a two-method synthetic AEF and manifest. Assert:
