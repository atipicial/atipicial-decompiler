...
**Tech Stack:** Rust 2021, existing CFG/SSA/structured IR, Atipicial opcodes, existing `tempfile` dev dependency, `dotnet`/Roslyn against an explicitly supplied Atipicial.SmartContract.Framework assembly. No production dependency additions.
...
Aggregate every successfully decompiled repository AEF and configured corpus item. Require no unclassified opcode, no unknown fallback reason, and a histogram entry for every incomplete method during migration. Add a final zero-fallback assertion initially marked with a clear test name, not ignored:
...
Use `ATC_SMARTCONTRACT_FRAMEWORK_DLL` and `dotnet` from `PATH`. Generate one temporary SDK project per case with a direct assembly reference:

```rust
let framework = std::env::var_os("ATC_SMARTCONTRACT_FRAMEWORK_DLL")
...
Ordinary `cargo test` portability is preserved by guarding the Rust integration test with `#[ignore = "requires dotnet and ATC_SMARTCONTRACT_FRAMEWORK_DLL"]`; the CI wrapper invokes it with `--ignored` and treats absence as setup failure.
...
```bash
ATC_SMARTCONTRACT_FRAMEWORK_DLL=/absolute/path/Atipicial.SmartContract.Framework.dll \
...
Text snapshots cannot detect undeclared symbols or invalid target typing, so representative typed output now builds through Roslyn against the Atipicial framework assembly.
...
- Roslyn case count and Atipicial.SmartContract.Framework assembly version/path used.
