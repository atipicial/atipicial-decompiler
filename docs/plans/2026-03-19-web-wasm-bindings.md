...
fn web_decompile_report_exposes_high_level_output_and_hashes() {
    let report = atipicial_decompiler::web::decompile_report(...).expect("web report");
...
```rust
#[wasm_bindgen]
pub fn decompile(aef_bytes: &[u8], options: JsValue) -> Result<JsValue, JsValue> { ... }
```
...
```js
import init, { decompile } from "./pkg/atipicial_decompiler.js";
```
