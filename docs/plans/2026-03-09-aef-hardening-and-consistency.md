...
**Goal:** Harden AEF parsing against malformed overlong varints and clean up core decompiler consistency issues so the crate is stricter, safer, and lint-clean.

**Architecture:** Keep the public API stable and tighten behavior at the low-level parsing boundary where malformed data enters the system. Add regression tests around malformed AEF payloads first, then implement the minimal parser change and small internal refactors that preserve behavior while improving consistency.

**Tech Stack:** Rust 2021, `cargo test`, `cargo clippy`, `cargo fmt`

### Task 1: Reject non-canonical AEF varints

**Files:**
- Modify: `src/error/aef.rs`
- Modify: `src/aef/encoding.rs`
- Test: `src/aef/tests/parse.rs`
- Test: `src/aef/tests/method_tokens/errors.rs`

**Step 1: Write the failing tests**

- Add a parser test that encodes an empty AEF source string using an overlong varint and expects a dedicated AEF parse error.
...
- Add a dedicated AEF error variant for non-canonical varints.
