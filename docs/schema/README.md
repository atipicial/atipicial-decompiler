...
| ------ | ------- | ----------- |
| `info.schema.json` | `atipicial-decompiler info --format json` | AEF metadata, manifest summary, method tokens (includes `returns_value`), warnings. |
| `disasm.schema.json` | `atipicial-decompiler disasm --format json` | Instruction stream including operand types/values and optional `returns_value` for `SYSCALL`. |
| `decompile.schema.json` | `atipicial-decompiler decompile --format json` | High-level/pseudocode listings plus the disassembly (including optional `returns_value`), manifest summary, method tokens (`returns_value`), and analysis (`call_graph`, `method_contracts`, `xrefs`, `types`). |
| `tokens.schema.json` | `atipicial-decompiler tokens --format json` | Standalone method-token listing (with `returns_value`). |
...
- The CLI can list schemas via `atipicial-decompiler schema --list` (each entry includes
  the semantic version, path, and a short description) or `atipicial-decompiler schema --list-json`
  for machine-readable listings, and print one via
  `atipicial-decompiler schema <info|disasm|decompile|tokens>`. Combine `--json-compact`
...
atipicial-decompiler info --format json hello.aef > report.json
