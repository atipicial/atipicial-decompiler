...
- `Example.aef` + `Example.manifest.json`
...
`atipicial-devpack-dotnet` v3.10.0 at its pinned commit, strictly extracts all 103
AEF/manifest pairs, and runs the same corpus checks. Reproduce the extraction
locally with:

```bash
python3 tools/extract_devpack_artifacts.py \
  --devpack-root /path/to/atipicial-devpack-dotnet \
...
`--devpack-root` is whatever revision your local `../atipicial-devpack-dotnet`
...
```bash
git -C ../atipicial-devpack-dotnet fetch origin tag v3.10.0
git -C ../atipicial-devpack-dotnet worktree add ../atipicial-decompiler/.devpack-v310 v3.10.0 --detach
...
`ATC_CSHARP_CORPUS_DIR` and `ATC_SMARTCONTRACT_FRAMEWORK_DLL` are set, so only
...
dotnet add .ci/roslyn package Atipicial.SmartContract.Framework --version 3.10.0
dotnet restore .ci/roslyn
export ATC_SMARTCONTRACT_FRAMEWORK_DLL="$HOME/.nuget/packages/atipicial.smartcontract.framework/3.10.0/lib/net10.0/Atipicial.SmartContract.Framework.dll"
export ATC_CSHARP_TARGET_FRAMEWORK=net10.0
export ATC_CSHARP_CORPUS_DIR="$PWD/TestingArtifacts/devpack"
