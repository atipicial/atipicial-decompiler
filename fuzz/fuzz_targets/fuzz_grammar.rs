...
use atipicial_decompiler::{Decompiler, AefParser};
...
/// Builds structurally valid AtipicialVM bytecode programs and wraps them in AEF.
...
impl ProgramBuilder {
    ...
    /// Wrap the bytecode in a valid AEF3 container with correct checksum.
    fn build_aef(&self) -> Vec<u8> {
        let mut aef = Vec::new();

        // Magic: "AEF3"
        aef.extend_from_slice(b"AEF3");
        ...
        aef.extend_from_slice(&compiler);
        ...
        aef.extend_from_slice(&0u16.to_le_bytes());

        // Script as varbytes
        let script_len = self.bytecode.len() as u32;
        write_varint(&mut aef, script_len);
        aef.extend_from_slice(&self.bytecode);

        // Checksum: double-SHA256, first 4 bytes as LE u32
        let checksum = AefParser::calculate_checksum(&aef);
        aef.extend_from_slice(&checksum.to_le_bytes());
...
fuzz_target!(|data: &[u8]| {
    ...
    // Wrap in AEF and run the full decompilation pipeline.
    let aef = builder.build_aef();
    let decompiler = Decompiler::new();
    let _ = decompiler.decompile_bytes(&aef);
});
