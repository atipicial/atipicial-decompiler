...
test("distinguishes void and value-returning known syscalls", () => {
  const storagePut = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x10, 0x10, 0x10, 0x41, 0xe6, 0x3f, 0x18, 0x84, 0x40])),
  );
  const checkWitness = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x10, 0x41, 0xf8, 0x27, 0xec, 0x8c, 0x40])),
  );
...
test("rewrites KEYS and VALUES as helper calls", () => {
  ...
  const keys = decompileHighLevelBytes(buildAefFromScript(keysScript));
  const values = decompileHighLevelBytes(buildAefFromScript(valuesScript));
...
test("rewrites APPEND and POPITEM using collection helpers", () => {
  ...
  const append = decompileHighLevelBytes(buildAefFromScript(appendScript));
  const popitem = decompileHighLevelBytes(buildAefFromScript(popitemScript));
