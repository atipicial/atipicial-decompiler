...
import {
  buildAefFromScript,
  buildAefWithSingleToken,
} from "./decompiler-fixtures.mjs";

test("builds call graph edges for syscalls, internal calls, CALLT, and CALLA", () => {
  const syscall = analyzeBytes(buildAefFromScript(new Uint8Array([0x41, 0xb7, 0xc3, 0x88, 0x03, 0x40])));
  ...
  const internal = analyzeBytes(buildAefFromScript(new Uint8Array([0x34, 0x05, 0x40, 0x21, 0x21, 0x57, 0x00, 0x00, 0x40])));
  ...
  const token = analyzeBytes(
    buildAefWithSingleToken(new Uint8Array([0x37, 0x00, 0x00, 0x40]), new Uint8Array(20), "transfer", 2, true, 0x0f),
  );
  ...
  const indirect = analyzeBytes(buildAefFromScript(new Uint8Array([0x11, 0x10, 0x36, 0x40])));
...
test("call graph: an out-of-range CALL target is unresolved, not a fabricated method", () => {
  ...
  const a = analyzeBytes(buildAefFromScript(new Uint8Array([0x34, 0x7f, 0x40])));
...
test("call graph: an out-of-range PUSHA+CALLA target is Indirect, not a fabricated method", () => {
  ...
  const a = analyzeBytes(
    buildAefFromScript(new Uint8Array([0x0a, 0x7f, 0x00, 0x00, 0x00, 0x36, 0x40])),
  );
...
test("includes slot xrefs for locals, arguments, and statics", () => {
  const local = analyzeBytes(buildAefFromScript(new Uint8Array([0x57, 0x01, 0x00, 0x11, 0x70, 0x68, 0x40])));
  assert.equal(local.xrefs.methods[0].locals[0].writes[0], 4);
  assert.equal(local.xrefs.methods[0].locals[0].reads[0], 5);

  const args = analyzeBytes(buildAefFromScript(new Uint8Array([0x57, 0x00, 0x02, 0x78, 0x11, 0x81, 0x40])));
  assert.equal(args.xrefs.methods[0].arguments[0].reads[0], 3);
  assert.equal(args.xrefs.methods[0].arguments[1].writes[0], 5);

  const statics = analyzeBytes(buildAefFromScript(new Uint8Array([0x56, 0x02, 0x11, 0x60, 0x58, 0x40])));
...
test("infers basic collection and manifest argument types", () => {
  const localMap = analyzeBytes(buildAefFromScript(new Uint8Array([0x57, 0x01, 0x00, 0xc8, 0x70, 0x68, 0x40])));
  assert.equal(localMap.types.methods[0].locals[0], "map");

  const staticMap = analyzeBytes(buildAefFromScript(new Uint8Array([0x56, 0x01, 0xc8, 0x60, 0x40])));
  ...
  const args = analyzeBytes(buildAefFromScript(new Uint8Array([0x57, 0x00, 0x02, 0x40])), manifest);
  assert.deepEqual(args.types.methods[0].arguments, ["integer", "bool"]);

  const inferredArgs = analyzeBytes(buildAefFromScript(new Uint8Array([0x78, 0x40])));
  assert.deepEqual(inferredArgs.types.methods[0].arguments, ["unknown"]);
});

test("infers PACKMAP and CONVERT target types", () => {
  const packMap = analyzeBytes(buildAefFromScript(new Uint8Array([0x57, 0x01, 0x00, 0x10, 0xbe, 0x70, 0x40])));
  assert.equal(packMap.types.methods[0].locals[0], "map");

  const packStruct = analyzeBytes(
    buildAefFromScript(new Uint8Array([0x57, 0x01, 0x00, 0x10, 0xbf, 0x70, 0x40])),
  );
  assert.equal(packStruct.types.methods[0].locals[0], "struct");

  const newBuffer = analyzeBytes(
    buildAefFromScript(new Uint8Array([0x57, 0x01, 0x00, 0x11, 0x88, 0x70, 0x40])),
  );
  assert.equal(newBuffer.types.methods[0].locals[0], "buffer");

  const convert = analyzeBytes(
    buildAefFromScript(new Uint8Array([0x57, 0x01, 0x00, 0x0c, 0x01, 0xaa, 0xdb, 0x28, 0x70, 0x40])),
  );
...
test("infers integer and boolean local types via stack simulation", () => {
  ...
  const intLiteral = analyzeBytes(
    buildAefFromScript(new Uint8Array([0x57, 0x01, 0x00, 0x15, 0x70, 0x40])),
  );
  ...
  const arithmetic = analyzeBytes(
    buildAefFromScript(new Uint8Array([0x57, 0x01, 0x00, 0x11, 0x11, 0x9e, 0x70, 0x40])),
  );
  ...
  const comparison = analyzeBytes(
    buildAefFromScript(new Uint8Array([0x57, 0x01, 0x00, 0x11, 0x13, 0xb5, 0x70, 0x40])),
  );
  assert.equal(comparison.types.methods[0].locals[0], "bool");

  // NEWARRAY0; SIZE; STLOC0 — SIZE yields an integer.
  const size = analyzeBytes(
    buildAefFromScript(new Uint8Array([0x57, 0x01, 0x00, 0xc2, 0xca, 0x70, 0x40])),
  );
...
test("inlineSingleUseTemps option inlines single-use temp variables", () => {
  ...
  const aef = buildAefFromScript(script);

  const without = decompileHighLevelBytes(aef);
  const with_ = decompileHighLevelBytes(aef, { inlineSingleUseTemps: true });
