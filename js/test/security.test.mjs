/**
 * Security Tests for Atipicial Decompiler JS
...
test("security: extremely large size fields (DoS prevention)", () => {
  ...
  for (const aef of cases) {
    assert.throws(() => parseAef(aef));
  }
});
...
test("security: no prototype pollution via manifest", () => {
  ...
  decompileHighLevelBytesWithManifest(aef, maliciousManifest);
...
test("security: stack depth limits in recursive structures", () => {
  ...
  const aef = buildValidAef(new Uint8Array(deepScript));
...
test("security: input validation on non-Uint8Array inputs", () => {
  ...
  assert.throws(() => parseAef("string"));
  assert.throws(() => parseAef(123));
  assert.throws(() => parseAef(null));
  assert.throws(() => parseAef(undefined));
  assert.throws(() => parseAef({}));
  assert.throws(() => parseAef([]));
});
