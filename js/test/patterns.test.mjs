...
function aef(compiler = "", source = "") {
  return { header: { compiler, source } };
}

function buildAef(script = [0x40], compiler = "Atipicial.Compiler.CSharp") {
  const data = [...Buffer.from("AEF3")];
...
test("pattern analysis treats declared AEP standards as authoritative", () => {
  const info = identifyPatterns(
    aef("Atipicial.Compiler.CSharp 3"),
    [],
    {
  ...
  assert.deepEqual(info.standards, ["AEP-17"]);
  assert.equal(info.language, "C#");
  assert.equal(info.confidence, "high");
  assert.ok(info.evidence.some((entry) => entry.source === "aef.header.compiler"));
});
...
test("pattern analysis accepts canonical raw manifest standard field names", () => {
  const info = identifyPatterns(
    aef(),
    [],
    { supportedstandards: ["AEP-17"], abi: { methods: [], events: [] } },
  );
  assert.deepEqual(info.standards, ["AEP-17"]);
  assert.deepEqual(info.patterns, ["AEP-17"]);
  assert.equal(info.confidence, "high");
});

test("basic JS decompile APIs expose the same pattern summary", () => {
  const bytes = buildAef();
  ...
  assert.deepEqual(withManifest.patterns.standards, ["AEP-17"]);
});

test("pattern analysis keeps weak source metadata explainable", () => {
  const info = identifyPatterns(aef("", "contract.py"), [], null);
  assert.deepEqual(info.standards, []);
  assert.equal(info.language, null);
  assert.equal(info.confidence, "low");
  assert.ok(info.evidence.some((entry) => entry.source === "aef.header.source"));
});
...
test("pattern analysis recognizes only supported C# source paths", () => {
  for (const source of ["C:\\contracts\\Token.cs", "/contracts/Token.csproj"]) {
    const info = identifyPatterns(aef("", source), [], null);
    assert.equal(info.language, "C#");
  }
});
...
test("pattern analysis does not claim unsupported source renderers", () => {
  for (const [compiler, source] of [
    ["boa 1", "contract.py"],
    ["atipicial-go 1", "contract.go"],
    ["atipicial-rustc 1", "contract.rs"],
    ["atipicial-java-compiler 1", "contract.java"],
    ["atipicial-javascript-compiler 1", "contract.ts"],
    ["Atipicial.Compiler.Rust 1", "contract.rs"],
    ["Atipicial.Compiler.Java 1", "contract.java"],
  ]) {
    const info = identifyPatterns(aef(compiler, source), [], null);
    assert.equal(info.language, null, `${compiler} ${source}`);
  }
});

test("pattern analysis requires explicit C# compiler tokens", () => {
  for (const compiler of ["notcsharp", "CSharpX", "Atipicial.Compiler.CSharpX", "cscompiler"]) {
    const info = identifyPatterns(aef(compiler), [], null);
...
test("pattern analysis mirrors tolerant wildcard method selectors", () => {
  ...
  const omitted = identifyPatterns(aef(), [], {
    supportedStandards: [],
    permissions: [{ contract: hashContract }],
    abi: { methods: [], events: [] },
  });
  const nonCanonical = identifyPatterns(aef(), [], {
    supportedStandards: [],
...
test("pattern analysis identifies token transfer behavior from paired ABI method and event", () => {
  const info = identifyPatterns(aef(), [], {
    supportedStandards: [],
    abi: {
...
test("pattern analysis identifies the AEP-24 royalty method", () => {
  const info = identifyPatterns(
    aef(),
    [],
    {
  ...
  assert.deepEqual(info.standards, ["AEP-24"]);
  assert.deepEqual(info.patterns, ["AEP-24", "royalties"]);
...
test("pattern analysis identifies token payment receiver callbacks without guessing a standard", () => {
  const info = identifyPatterns(aef(), [], {
    name: "Receiver",
    abi: {
      methods: [{ name: "onAEP17Payment", parameters: [], returntype: "Void" }],
      events: [],
    },
  });
  assert.deepEqual(info.standards, []);
  ...
  assert.ok(info.evidence.some((entry) =>
    entry.source === "manifest.abi.methods" && entry.value === "onAEP17Payment"
  ));
});
...
test("pattern analysis identifies external-call bytecode signals", () => {
  const info = identifyPatterns(
    { ...aef(), methodTokens: [{ method: "transfer" }] },
...
test("pattern evidence ordering matches Rust codepoint sorting", () => {
  ...
  assert.deepEqual(
    info.evidence
      .filter((entry) => entry.source === "aef.method_tokens.native")
...
test("pattern analysis identifies native system-management contracts", () => {
  const cases = [
    ...
    ["c13a56c98353a7ea6a324d9a835d1b5bf2266315", "OnAEP11Payment", "treasury"],
  ];
  for (const [hex, method, pattern] of cases) {
    const info = identifyPatterns(
      { ...aef(), methodTokens: [{ hash: Uint8Array.from(Buffer.from(hex, "hex")), method }] },
      [],
      null,
    );
...
test("pattern analysis identifies fine-grained native behavior", () => {
  ...
  for (const [hex, method, pattern] of cases) {
    ...
    assert.ok(info.patterns.includes(pattern), `${method} should identify ${pattern}`);
    assert.ok(info.evidence.some((entry) =>
      entry.source === "aef.method_tokens.pattern" &&
...
test("high-level C# output surfaces its inferred pattern summary", () => {
  ...
  assert.match(result.csharp, /    \/\/ inferred standards: AEP-17/);
  assert.match(result.csharp, /    \/\/ inferred patterns: AEP-17/);
...
test("C# method-token rewriting leaves qualified native calls intact", () => {
  const rendered = renderCSharpContract([
    "contract NativeCall {",
    "fn invoke() -> any {",
    "    return AtipicialCoin.Transfer(from, to, amount);",
    "}",
    "}",
  ].join("\n"), null, {
    methodTokens: [{
  ...
  assert.match(rendered, /return AtipicialCoin\.Transfer\(@from, to, amount\);/);
...
test("C# rendering falls back to Contract.Call for non-framework natives", () => {
  ...
  const rendered = renderCSharpContract([
    ...
    "    let ok = AtipicialCoin::UnregisterCandidate(pubkey);",
    "    return ok;",
    "}",
    "}",
  ].join("\n"), null, {
  ...
  assert.match(rendered, /bool ok = AtipicialCoin\.UnRegisterCandidate\(pubkey\);/);
});

test("C# rendering keeps known void natives and syscalls statement-only", () => {
  const rendered = renderCSharpContract([
    "contract VoidCalls {",
    "fn main(aef, manifest, data, url, filter, cb, user, gas) -> void {",
    "    let destroyed = ContractManagement::Destroy();",
    "    let updated = ContractManagement::Update(aef, manifest, data);",
  ...
  assert.match(rendered, /^\s*ContractManagement\.Update\(aef, manifest, data\);$/m);
...
test("C# rendering recognizes additional Atipicial runtime and crypto syscalls", () => {
  const source = [
    "contract Runtime {",
    'fn inspect() -> any {',
...
test("C# rendering emits manifest class attributes", () => {
  const csharp = renderCSharpContract(
    "contract Token {\n}",
    {
      supportedStandards: ["AEP-17", "AEP-11"],
      extra: { Email: "owner@example.com", Version: 2, Nested: { ignored: true } },
    },
  );
  assert.match(csharp, /\[SupportedStandards\("AEP-17", "AEP-11"\)\]/);
...
test("C# rendering lowers common Atipicial math and byte helpers", () => {
  const csharp = renderCSharpContract([
    "contract Token {",
...
test("C# rendering lowers Atipicial concatenation outside string literals", () => {
  const csharp = renderCSharpContract([
    "contract Token {",
    "fn join(a, b) -> string {",
...
test("C# rendering guards non-void VM fallthrough after structured blocks", () => {
  ...
  assert.match(csharp, /\/\/ unreachable VM fallthrough\n\s+throw new InvalidOperationException\("Unreachable Atipicial VM fallthrough\."\);/);
  assert.doesNotMatch(csharp, /public static void run\(\)[\s\S]*Unreachable Atipicial VM fallthrough/);
});
...
test("C# rendering converts value-less returns only in non-void methods", () => {
  ...
  assert.match(csharp, /public static bool @value\(\)[\s\S]*throw new InvalidOperationException\("Unreachable Atipicial VM fallthrough\."\);/);
...
test("C# rendering lowers native qualified calls outside literals", () => {
  const csharp = renderCSharpContract([
    "contract Token {",
    "fn call() -> any {",
    "    let value = AtipicialDollar::Transfer(from, to, amount);",
    "    let gasSymbol = AtipicialDollar::Symbol();",
    "    let atipicialDecimals = AtipicialCoin::Decimals();",
    ...
    '    return "AtipicialDollar::Transfer(x)";',
    "}",
    "}",
  ].join("\n"));
  assert.match(csharp, /AtipicialDollar\.Transfer\(@from, to, amount\)/);
  assert.match(csharp, /AtipicialDollar\.Symbol;/);
  assert.match(csharp, /AtipicialCoin\.Decimals;/);
  ...
  assert.doesNotMatch(
    csharp,
    /(?:AtipicialDollar\.Symbol|AtipicialCoin\.Decimals|LedgerContract\.CurrentHash|LedgerContract\.CurrentIndex)\(\)/,
  );
  assert.match(csharp, /AtipicialDollar::Transfer\(x\)/);
});
...
test("C# rendering declares referenced static VM slots", () => {
  ...
  assert.match(csharp, /private static BigInteger __atipicialStatic0;/);
  assert.match(csharp, /private static BigInteger __atipicialStatic1;/);
  assert.match(csharp, /__atipicialStatic0 = __atipicialStatic1;/);
  assert.match(csharp, /return __atipicialStatic0;/);
...
test("C# static slot inference stays dynamic for conflicting writes", () => {
  ...
  assert.match(csharp, /private static dynamic __atipicialStatic0;/);
});
