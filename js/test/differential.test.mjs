...
// .exe suffix), contributors can point at any build via ATC_DECOMPILER_BIN.
const RUST_BIN =
  process.env.ATC_DECOMPILER_BIN ??
  join(ROOT, "target", "release", process.platform === "win32" ? "atipicial-decompiler.exe" : "atipicial-decompiler");
...
function findAefFiles(dir) {
  const results = [];
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    const full = join(dir, entry.name);
    if (entry.isDirectory()) {
      results.push(...findAefFiles(full));
    } else if (entry.name.endsWith(".aef")) {
      results.push(full);
    }
  }
  return results;
...
function runRust(subcommand, aefPath, options = []) {
  try {
    return execFileSync(RUST_BIN, [subcommand, ...options, aefPath], {
      encoding: "utf-8",
      timeout: 15_000,
...
/**
 * Find the manifest file path that matches a .aef file (same stem + .manifest.json).
 */
function findManifestForAef(aefPath) {
  const stem = aefPath.replace(/\.aef$/, "");
...
const aefFiles = findAefFiles(ARTIFACTS_DIR);

test("differential: found test artifacts", () => {
  assert.ok(aefFiles.length > 0, `expected .aef files in ${ARTIFACTS_DIR}`);
});
...
for (const aefPath of aefFiles) {
  const label = relative(ROOT, aefPath);

  test(`differential disasm: ${label}`, () => {
    const bytes = readFileSync(aefPath);

    // Rust disassembly
    const rustRaw = runRust("disasm", aefPath);
...
for (const aefPath of aefFiles) {
  ...
  test(`differential high-level: ${label}`, () => {
    ...
    const rustRaw = runRust("decompile", aefPath, ["--format", "high-level"]);
    if (typeof rustRaw === "object" && rustRaw.error) {
    ...
    const manifestPath = findManifestForAef(aefPath);
    let jsResult;
    try {
      if (manifestPath) {
...
function runRustJson(aefPath) {
  try {
    const out = execFileSync(RUST_BIN, ["decompile", "--format", "json", aefPath], {
      encoding: "utf-8",
      timeout: 15_000,
      stdio: ["pipe", "pipe", "pipe"],
    });
...
for (const aefPath of aefFiles) {
  ...
  test(`differential analysis: ${label}`, () => {
    const rust = runRustJson(aefPath);
    if (!rust || !rust.analysis) {
      return; // Rust binary unavailable, or it could not decompile this AEF.
    }
