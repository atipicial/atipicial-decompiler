...
const corpus = process.env.ATC_CSHARP_CORPUS_DIR;
const framework = process.env.ATC_SMARTCONTRACT_FRAMEWORK_DLL;
const skipReason = !corpus
  ? "requires ATC_CSHARP_CORPUS_DIR"
  : !framework
    ? "requires ATC_SMARTCONTRACT_FRAMEWORK_DLL"
    : false;

test("pinned JS-generated C# corpus compiles with Roslyn", { skip: skipReason }, () => {
  const targetFramework = process.env.ATC_CSHARP_TARGET_FRAMEWORK ?? "net8.0";
  assert.match(
    targetFramework,
    /^net[0-9A-Za-z.]+$/,
    "ATC_CSHARP_TARGET_FRAMEWORK must be a target moniker such as net8.0",
  );
  ...
  assert.equal(
    ...
    "JS C# corpus must use the pinned atipicial-devpack-dotnet revision",
  );

  const aefFiles = readdirSync(corpus)
    .filter((name) => name.endsWith(".aef"))
    .sort();
  assert.equal(aefFiles.length, 103, "pinned JS C# corpus count drift");

  const project = mkdtempSync(join(tmpdir(), "atipicial-js-csharp-corpus-"));
  try {
    writeFileSync(
      join(project, "Generated.csproj"),
      `<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>${targetFramework}</TargetFramework><Nullable>disable</Nullable><ImplicitUsings>disable</ImplicitUsings></PropertyGroup><ItemGroup><Reference Include="Atipicial.SmartContract.Framework"><HintPath>${escapeXml(framework)}</HintPath></Reference></ItemGroup></Project>`,
    );
    ...
    for (const aefName of aefFiles) {
      ...
      const allowedWarnings = aefName === "Contract_Foreach.aef"
      ...
      for (const warning of decompiled.warnings) {
        if (!hazardWarnings.includes(warning) && !allowedWarnings.includes(warning)) {
          unexpectedWarnings.push({ aefName, warning });
        }
      }
