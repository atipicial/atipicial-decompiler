/**
 * Performance and Stress Tests for Atipicial Decompiler JS
 */
...
test("performance: small contract decompilation", () => {
  const aef = buildValidAef(new Uint8Array([
    0x57, 0x01, 0x01,
    0x78, 0x11, 0x9e, 0x70,
    0x68, 0x40,
  ]));
...
test("stress: rapid sequential processing", () => {
  const aef = buildValidAef(new Uint8Array([0x57, 0x01, 0x01, 0x78, 0x11, 0x9e, 0x70, 0x68, 0x40]));
...
test("stress: memory stability over many iterations", () => {
  const aef = buildValidAef(new Uint8Array(5000).fill(0x21));
...
test("stress: concurrent processing", async () => {
  const scripts = [
    buildValidAef(new Uint8Array([0x11, 0x40])),
    buildValidAef(new Uint8Array([0x12, 0x40])),
    buildValidAef(new Uint8Array([0x13, 0x40])),
  ];
  
  const promises = [];
  for (let i = 0; i < 100; i++) {
    promises.push(new Promise((resolve) => {
      const aef = scripts[i % 3];
