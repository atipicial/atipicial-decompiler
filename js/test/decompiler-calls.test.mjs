...
test("PUSHINT128 / PUSHINT256 decode as decimal literals", () => {
  ...
  const positive = decompileHighLevelBytes(buildAefFromScript(positiveScript));
  ...
  const negative = decompileHighLevelBytes(buildAefFromScript(negScript));
...
test("emits method-token header with native-contract description", () => {
  // Build a AEF whose method tokens table contains a single entry pointing
  ...
  // Compose the AEF by hand: PUSH1; RET script with a single token row.
  const tokenRow = [];
  ...
  const data = [];
  for (const c of "AEF3") data.push(c.charCodeAt(0));
...
test("uses method token metadata to lift CALLT arguments and void returns", () => {
  const aef = buildAefWithSingleToken(
...
test("uses call-graph-resolved CALLA targets across method arguments", () => {
  ...
  const analysis = analyzeBytes(aef, manifest);
  ...
  const { highLevel } = decompileHighLevelBytesWithManifest(aef, manifest);
  assert.match(
    highLevel,
...
test("method contracts classify a private void helper and preserve the caller return", () => {
  ...
  const result = decompileHighLevelBytesWithManifest(aef, manifest, {
...
test("method contracts keep recursive, mixed, and missing private returns unknown", () => {
  ...
  const missing = analyzeBytes(
    buildAefFromScript(new Uint8Array([0x34, 0x04, 0x40, 0x21, 0x38])),
    manifest,
  );
...
test("method contracts keep manifest return declarations authoritative", () => {
  ...
  const value = analyzeBytes(aef, withHelperReturn("Integer"));
  const voidResult = analyzeBytes(aef, withHelperReturn("Void"));
