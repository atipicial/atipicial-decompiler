/**
 * Deep Fuzz Tests for Atipicial Decompiler JS
 *
...
function buildValidAef(script, tokens = []) {
  const data = [];
...
test("deep-fuzz: random if-else chains", () => {
  for (let iter = 0; iter < 100; iter++) {
    ...
    mustNotCrash(() => decompileHighLevelBytes(aef));
  }
});
...
test("deep-fuzz: CALLT with method tokens", () => {
  for (let iter = 0; iter < 50; iter++) {
    ...
    const aef = buildValidAef(new Uint8Array(script), tokens);
    mustNotCrash(() => {
      const result = analyzeBytes(aef);
...
test("deep-fuzz: manifest with extreme/unusual field values", () => {
  const edgeCases = [
    ...
    // Supported standards
    { name: "C", supportedstandards: ["AEP-17", "AEP-11", "AEP-27", "Custom-1"] },
  ...
  for (const manifest of edgeCases) {
    mustNotCrash(() => {
      decompileHighLevelBytesWithManifest(aef, JSON.stringify(manifest));
    });
  }
});
...
// ─── 9. AEF structure mutation fuzzing ──────────────────────────────────────

test("deep-fuzz: AEF with every byte position flipped", () => {
  const base = buildValidAef(new Uint8Array([0x11, 0x12, 0x9e, 0x40]));

  for (let i = 0; i < base.length; i++) {
    const mutated = new Uint8Array(base);
    mutated[i] ^= 0xff; // flip all bits at position i
    mustNotCrash(() => parseAef(mutated));
  }
});

test("deep-fuzz: AEF with random single-byte mutations", () => {
  const base = buildValidAef(new Uint8Array([
  ...
  for (let iter = 0; iter < 200; iter++) {
    ...
    mustNotCrash(() => {
      const aef = parseAef(mutated);
      disassembleScript(aef.script);
    });
  }
});

test("deep-fuzz: AEF with multi-byte mutations", () => {
  const base = buildValidAef(new Uint8Array([
    0x57, 0x02, 0x01,
...
test("deep-fuzz: AEF with compiler field containing all byte values", () => {
  for (let iter = 0; iter < 30; iter++) {
    ...
    mustNotCrash(() => parseAef(new Uint8Array(data)));
  }
});

test("deep-fuzz: AEF with source string of varying lengths", () => {
...
test("deep-fuzz: realistic contract patterns (transfer-like)", () => {
  for (let iter = 0; iter < 40; iter++) {
    ...
    mustNotCrash(() => {
      const result = decompileHighLevelBytesWithManifest(aef, JSON.stringify(manifest));
...
test("deep-fuzz: output determinism across repeated runs", () => {
  for (let iter = 0; iter < 30; iter++) {
    ...
    let result1, result2;
    try {
      result1 = decompileHighLevelBytes(aef);
      result2 = decompileHighLevelBytes(aef);
    } catch {
...
test("deep-fuzz: full analysis pipeline with random manifests", () => {
  for (let iter = 0; iter < 40; iter++) {
    ...
    const manifest = {
      name: `Contract_${iter}`,
      abi: { methods, events: [] },
      supportedstandards: randomInt(0, 1) ? ["AEP-17"] : [],
      permissions: [],
      trusts: [],
      extra: { author: "fuzz" },
    };

    const aef = buildValidAef(new Uint8Array(scriptParts));
    mustNotCrash(() => {
      const result = decompileHighLevelBytesWithManifest(aef, JSON.stringify(manifest));
    ...
    mustNotCrash(() => {
      const result = analyzeBytes(aef, JSON.stringify(manifest));
      assert.ok(result.callGraph);
      assert.ok(result.xrefs);
      assert.ok(result.types);
    });
...
test("deep-fuzz: CONVERT and ISTYPE with all possible type bytes", () => {
  for (let typeByte = 0; typeByte <= 0xff; typeByte++) {
    ...
    const aef1 = buildValidAef(new Uint8Array(script1));
    const aef2 = buildValidAef(new Uint8Array(script2));
    mustNotCrash(() => decompileHighLevelBytes(aef1));
    mustNotCrash(() => decompileHighLevelBytes(aef2));
  }
});
