/**
 * Real TestingArtifacts AEF+manifest fixtures must decompile to high-level
 * Atipicial C# SmartContract source with pattern headers (parity with Rust gates).
 */
...
function collectAefs(dir, out = []) {
  if (!existsSync(dir)) return out;
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    const path = join(dir, entry.name);
    if (entry.isDirectory()) {
      if (entry.name === "decompiled") {
        collectAefs(join(path, "embedded"), out);
        continue;
      }
      collectAefs(path, out);
      continue;
    }
    if (entry.name.endsWith(".aef")) out.push(path);
  }
  return out;
}
...
test("all supported TestingArtifacts decompile to high-level C# contracts", () => {
  ...
  const aefs = collectAefs(ARTIFACTS).sort();
  assert.ok(aefs.length > 0, "expected TestingArtifacts AEFs");

  let checked = 0;
  for (const aefPath of aefs) {
    const rel = relative(ARTIFACTS, aefPath).replaceAll("\\", "/");
    if (skips.some((skip) => rel.includes(skip))) continue;

    const stem = aefPath.slice(0, -".aef".length);
    const manifestPath = `${stem}.manifest.json`;
    assert.ok(existsSync(manifestPath), `manifest for ${rel}`);

    const bytes = new Uint8Array(readFileSync(aefPath));
