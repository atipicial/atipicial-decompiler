/**
 * Boundary and Edge Case Tests for Atipicial Decompiler JS
...
test("boundary: single byte script", () => {
  ...
  const aef = buildAef({ script: new Uint8Array([0x40]) });
...
test("boundary: max short varint (0xfc = 252)", () => {
  ...
  assert.doesNotThrow(() => parseAef(new Uint8Array(data)));
});
...
test("boundary: max 2-byte varint exceeds source limit and is rejected", () => {
  ...
  assert.throws(() => parseAef(new Uint8Array(data)), /exceeds/);
});

test("boundary: all PUSH0-PUSH16 opcodes", () => {
  for (let i = 0; i <= 16; i++) {
    const opcode = 0x10 + i; // PUSH0 = 0x10, PUSH16 = 0x20
    const aef = buildAef({ script: new Uint8Array([opcode, 0x40]) });
    const result = decompileBytes(aef);
...
test("boundary: all jump variants", () => {
  ...
  for (const { opcode, name } of jumps) {
    ...
    const aef = buildAef({ script });
    assert.doesNotThrow(() => decompileHighLevelBytes(aef), `${name} should not crash`);
  }
});
...
test("boundary: all slot opcodes (0-6)", () => {
  ...
  for (const group of opcodes) {
    for (const opcode of group) {
      const aef = buildAef({ script: new Uint8Array([opcode, 0x40]) });
      assert.doesNotThrow(() => decompileHighLevelBytes(aef), `opcode 0x${opcode.toString(16)} should not crash`);
    }
  }
});
...
test("boundary: indexed slot opcodes (LDLOC, STLOC, etc.)", () => {
  ...
  const aef = buildAef({ script });
  assert.doesNotThrow(() => decompileHighLevelBytes(aef));
});
...
test("boundary: arithmetic opcodes", () => {
  ...
  for (const opcode of arithOps) {
    const aef = buildAef({ script: new Uint8Array([0x11, 0x12, opcode, 0x40]) });
    assert.doesNotThrow(() => decompileHighLevelBytes(aef), `opcode 0x${opcode.toString(16)} should not crash`);
  }
});
...
test("boundary: max script length (within limits)", () => {
  ...
  const aef = buildAef({ script: largeScript });
  assert.doesNotThrow(() => analyzeBytes(aef));
});
...
test("boundary: zero-sized data PUSHes", () => {
  ...
  const aef = buildAef({ script });
  assert.doesNotThrow(() => decompileBytes(aef));
});
...
test("boundary: all TRY variants", () => {
  ...
  assert.doesNotThrow(() => decompileHighLevelBytes(buildAef({ script: tryShort })));
  ...
  assert.doesNotThrow(() => decompileHighLevelBytes(buildAef({ script: tryLong })));
});

test("boundary: exception handling edge cases", () => {
  // THROW without try block
  const throwScript = new Uint8Array([0x11, 0x3a, 0x40]);
  assert.doesNotThrow(() => decompileHighLevelBytes(buildAef({ script: throwScript })));
  
  // ABORT
  const abortScript = new Uint8Array([0x38, 0x40]);
  assert.doesNotThrow(() => decompileHighLevelBytes(buildAef({ script: abortScript })));
  
  // ASSERT
  const assertScript = new Uint8Array([0x11, 0x39, 0x40]);
  assert.doesNotThrow(() => decompileHighLevelBytes(buildAef({ script: assertScript })));
});

test("boundary: ENDFINALLY without preceding TRY", () => {
  const script = new Uint8Array([0x3f, 0x40]); // ENDFINALLY, RET
  assert.doesNotThrow(() => decompileHighLevelBytes(buildAef({ script })));
});
...
test("boundary: method name longer than 32 bytes is rejected", () => {
  ...
  assert.throws(() => parseAef(aef), (error) => {
    assert.ok(error instanceof AefParseError);
    assert.equal(error.details.code, "InvalidMethodToken");
    return true;
  });
});

test("boundary: max method tokens (128 per AefFile.Deserialize)", () => {
  ...
  const parsed = parseAef(buildAef({ tokens: makeTokens(128), script }));
  assert.equal(parsed.methodTokens.length, 128);

  assert.throws(() => parseAef(buildAef({ tokens: makeTokens(129), script })), (error) => {
    assert.ok(error instanceof AefParseError);
...
test("boundary: all syscall hashes", () => {
  ...
  for (const { hash, name } of syscalls) {
    ...
    const result = decompileHighLevelBytes(buildAef({ script }));
...
test("boundary: reserved bytes must be zero", () => {
  ...
  assert.throws(() => parseAef(new Uint8Array(data)));
});
...
test("boundary: non-canonical varint encoding is accepted (matches MemoryReader.ReadVarInt)", () => {
  ...
  // and AEF checksums cover the raw bytes, so such files are valid
  ...
  const parsed = parseAef(new Uint8Array(data));
...
test("boundary: very deep operand stack simulation", () => {
  ...
  assert.doesNotThrow(() => decompileHighLevelBytes(buildAef({ script: new Uint8Array(script) })));
});
