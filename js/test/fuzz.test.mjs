/**
 * Fuzz Tests for Atipicial Decompiler JS
...
test("fuzz: random byte sequences should not crash parseAef", () => {
  for (let i = 0; i < 100; i++) {
...
test("fuzz: random valid-length AEF with wrong checksum", () => {
  for (let i = 0; i < 50; i++) {
...
test("fuzz: truncated AEF at various positions", () => {
  const full = buildValidAef(new Uint8Array([0x11, 0x40]));
...
test("fuzz: extra/trailing bytes after AEF", () => {
  const full = buildValidAef(new Uint8Array([0x11, 0x40]));
  const extra = new Uint8Array([...full, 0xde, 0xad, 0xbe, 0xef]);
  
  assert.throws(() => parseAef(extra));
});
...
test("fuzz: recursive/nested structures depth", () => {
  ...
  const aef = buildValidAef(new Uint8Array(deepIfScript));
  try {
    decompileHighLevelBytes(aef);
  } catch (e) {
    assert.ok(e instanceof Error);
...
test("fuzz: edge case - empty input", () => {
  assert.throws(() => parseAef(new Uint8Array(0)));
});

test("fuzz: edge case - single byte input", () => {
  assert.throws(() => parseAef(new Uint8Array([0x00])));
});

test("fuzz: edge case - only magic bytes", () => {
  assert.throws(() => parseAef(new Uint8Array([0x4e, 0x45, 0x46, 0x33]))); // "AEF3"
});
