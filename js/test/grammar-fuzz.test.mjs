/**
 * Grammar-based fuzz tests for Atipicial Decompiler JS
 *
 * Generates STRUCTURALLY VALID AtipicialVM programs using a seeded PRNG
...
// ---- AtipicialVM opcode bytes (from js/src/generated/opcodes.js) ----
...
function runAllDecompilers(aef, label) {
  mustNotCrash(() => decompileBytes(aef), `${label} decompileBytes`);
  mustNotCrash(() => decompileHighLevelBytes(aef), `${label} decompileHighLevelBytes`);
  mustNotCrash(() => analyzeBytes(aef), `${label} analyzeBytes`);
}
...
test("Grammar Fuzz: try/catch programs (50 iterations)", async (t) => {
  for (let i = 0; i < 50; i++) {
    await t.test(`trycatch seed=${i}`, () => {
      ...
      const aef = buildAef(script);
      runAllDecompilers(aef, `trycatch[${i}]`);
    });
  }
});
...
test("Grammar Fuzz: collection programs (100 iterations)", async (t) => {
  for (let i = 0; i < 100; i++) {
    await t.test(`collection seed=${i}`, () => {
      ...
      const aef = buildAef(script);
      runAllDecompilers(aef, `collection[${i}]`);
    });
  }
});
...
test("Grammar Fuzz: extreme valid programs (50 iterations)", async (t) => {
  for (let i = 0; i < 50; i++) {
    await t.test(`extreme seed=${i}`, () => {
      ...
      const aef = buildAef(script);
      runAllDecompilers(aef, `extreme[${i}]`);
    });
  }
});
