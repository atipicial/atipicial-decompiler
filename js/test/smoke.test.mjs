/**
 * Smoke Tests for Atipicial Decompiler JS
...
function buildAefFromScript(scriptBytes) {
...
test("SMOKE: Basic AEF parsing does not crash", () => {
  const aef = buildAefFromScript(new Uint8Array([0x11, 0x40]));
  assert.doesNotThrow(() => parseAef(aef));
});

test("SMOKE: Basic disassembly does not crash", () => {
  const aef = buildAefFromScript(new Uint8Array([0x11, 0x12, 0x9e, 0x40]));
  assert.doesNotThrow(() => disassembleScript(aef.slice(-8, -4))); // script portion
});
...
test("SMOKE: Empty script handled gracefully", () => {
  // Note: Empty scripts are invalid per AEF spec, should throw
  const data = [];
  data.push(...Buffer.from("AEF3"));
  const compiler = new Uint8Array(64);
  data.push(...compiler);
...
test("SMOKE: Large script handled without crash", () => {
  ...
  const aef = buildAefFromScript(largeScript);
...
test("SMOKE: Deeply nested control flow handled", () => {
  ...
  const aef = buildAefFromScript(new Uint8Array(script));
  assert.doesNotThrow(() => decompileHighLevelBytes(aef));
});

test("SMOKE: Many method tokens handled", () => {
  // This would require token support in buildAefFromScript
...
test("SMOKE: All major opcodes can be disassembled", () => {
  ...
  for (const { opcode, extra } of opcodeTests) {
    ...
    assert.doesNotThrow(() => decompileBytes(aef), `opcode 0x${opcode.toString(16)} should not crash`);
  }
});

test("SMOKE: Invalid checksum properly rejected", () => {
  const aef = buildAefFromScript(new Uint8Array([0x11, 0x40]));
  aef[aef.length - 1] ^= 0xff; // Corrupt checksum
  assert.throws(() => parseAef(aef), /checksum/);
});

test("SMOKE: Invalid magic properly rejected", () => {
  const aef = buildAefFromScript(new Uint8Array([0x11, 0x40]));
  aef[0] = 0x58; // 'X' instead of 'N'
  assert.throws(() => parseAef(aef), /magic/);
});
...
test("SMOKE: Concurrent processing safe", async () => {
  ...
  const aefs = scripts.map(buildAefFromScript);
  
  // Process all concurrently
  const results = await Promise.all(
    aefs.map(aef => Promise.resolve(decompileHighLevelBytes(aef)))
  );
...
test("SMOKE: Output is deterministic", () => {
  const aef = buildAefFromScript(new Uint8Array([
    0x57, 0x01, 0x00,
    0x11, 0x70, 0x68, 0x40,
  ]));
  
  const result1 = decompileHighLevelBytes(aef);
  const result2 = decompileHighLevelBytes(aef);
...
test("SMOKE: manifest method offset/safe strict typing (matches Rust)", () => {
  ...
  // Negative offsets still treated as "no offset" (Atipicial -1 convention) — both match.
  assert.doesNotThrow(() =>
    parseManifest(
      JSON.stringify({
        name: "C",
...
test("SMOKE: manifest abi and feature/standards strict typing (matches Rust)", () => {
  ...
  // ( requires it empty — strict mode rejects non-empty, see the
  // strict-mode tests).
  assert.throws(
    () =>
      parseManifest(
