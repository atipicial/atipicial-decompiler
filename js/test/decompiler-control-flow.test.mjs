...
test("lifts locals into named high-level statements", () => {
  const result = decompileHighLevelBytes(buildLocalMathAef());
...
test("uses manifest method names in grouped high-level output", () => {
  const result = decompileHighLevelBytesWithManifest(buildSampleAef(), SAMPLE_MANIFEST);
...
test("manifest-less header runs `// manifest not provided` flush against `// method tokens declared in AEF`", () => {
  ...
  // method tokens declared in AEF`. Rust runs them flush. Verify
  ...
  const { highLevel } = decompileHighLevelBytes(aef);
  assert.match(
    highLevel,
    /\/\/ manifest not provided\n {4}\/\/ method tokens declared in AEF/,
    "manifest-not-provided and method-tokens header should be flush:\n" +
      highLevel,
  );
});

test("manifest-less header surfaces `// manifest not provided` and uses AtipicialContract default", () => {
  // Without a manifest the Rust port emits `contract AtipicialContract { ...
  ...
  assert.match(
    result.highLevel,
    /^contract AtipicialContract \{/m,
    "default contract name should be `AtipicialContract` (matches Rust)",
  );
  assert.match(
    result.highLevel,
    /\/\/ manifest not provided/,
