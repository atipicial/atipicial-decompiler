...
test("PUSHA pushes function-pointer expression `&fn_0xNNNN` (Rust parity)", async () => {
  ...
  const aef = buildAefFromScript(script);
  const { highLevel } = decompileHighLevelBytes(aef, { clean: true });
...
test("inferred helper labels use uppercase hex (Rust `sub_0x{:04X}` parity)", async () => {
  const { decompileHighLevelBytes } = await import("../src/index.js");
  // Build a AEF with seven RETs after the 4-byte entry, putting a sub
...
test("extractContractName mirrors Rust extract_contract_name fallback to AtipicialContract", async () => {
  ...
  assert.equal(extractContractName({ name: "   " }), "AtipicialContract");
  ...
  // `"AtipicialContract"` — that fallback only kicks in when the trimmed
  // input is itself empty).
  assert.equal(extractContractName({ name: "@@@" }), "param");
  // Null / undefined / missing manifest → fallback.
  assert.equal(extractContractName(null), "AtipicialContract");
  assert.equal(extractContractName(undefined), "AtipicialContract");
  assert.equal(extractContractName({}), "AtipicialContract");
...
test("unstructured ENDFINALLY is silently consumed (parity with Rust clean mode)", () => {
  ...
  const { highLevel, warnings } = decompileHighLevelBytes(buildAefFromScript(script));
...
test("untranslated opcode surfaces a structured warning (not just an inline comment)", () => {
  // 0xFF is reserved/unknown in ATC. The disassembler emits
...
test("detached PACKSTRUCT helpers infer entry arity and keep C# underflow explicit", () => {
  ...
  const { highLevel, csharp, warnings } = decompileHighLevelBytes(buildAefFromScript(script), {
