/**
 * Integration Tests for Atipicial Decompiler JS
...
test("integration: complete workflow from bytes to analysis output", () => {
  ...
  assert.ok(result.aef, "should parse AEF");
  ...
  const hlResult = decompileHighLevelBytes(aef);
  ...
  assert.match(hlResult.csharp, /public class AtipicialContract/, "should expose C# rendering");
});
...
test("integration: C# declarations are typed by default with an explicit legacy opt-out", () => {
  ...
  const typed = decompileHighLevelBytes(aef).csharp;
  const legacy = decompileHighLevelBytes(aef, { typedDeclarations: false }).csharp;
...
test("integration: C# rendering translates manifest signatures and preserves body structure", () => {
  ...
  const result = decompileHighLevelBytesWithManifest(
    buildAef({ script: [0x78, 0x40], compiler: "Atipicial.Compiler.CSharp" }),
    manifest,
  );
...
test("integration: multi-method contract with complex control flow", () => {
  ...
  const result = analyzeBytes(buildAef({ script }), manifest);
...
test("integration: syscall-heavy contract analysis", () => {
  ...
  const result = analyzeBytes(buildAef({ script }));
