...
test("tables: AtipicialCoin exposes getCandidates across all three committed files", () => {
  const atipicial = nativeJson.find((c) => c.name === "AtipicialCoin");
  assert.ok(atipicial, "AtipicialCoin present in json");
  assert.ok(atipicial.methods.includes("GetCandidates"), "json AtipicialCoin lists GetCandidates");
