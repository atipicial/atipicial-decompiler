/**
 * Systematic Fuzz Tests for Atipicial Decompiler JS
 *
 * Covers:
...
function buildAef(opts = {}) {
  const data = [];
  data.push(...Buffer.from("AEF3"));
...
function buildValidAef(script) {
  return buildAef({ script });
}
...
test("opcode-matrix: every opcode individually through decompileBytes", () => {
  for (const op of ALL_OPCODES) {
    const scriptBytes = [op.byte, ...operandForEncoding(op.encoding), 0x40];
    const aef = buildValidAef(new Uint8Array(scriptBytes));
    mustNotCrash(
      () => decompileBytes(aef),
...
test("opcode-matrix: all pairs of no-operand opcodes", () => {
  ...
  for (let i = 0; i < 500; i++) {
    ...
    const aef = buildValidAef(script);
    mustNotCrash(
      () => decompileHighLevelBytes(aef),
...
test("stack-depth: push 1000 values then consume", () => {
  ...
  const aef = buildValidAef(new Uint8Array(script));
  mustNotCrash(() => decompileHighLevelBytes(aef), "deep push + consume");
});
...
test("stack-depth: push 2000 values, no consumption (unbalanced)", () => {
  ...
  script.push(0x40); // RET
  const aef = buildValidAef(new Uint8Array(script));
  mustNotCrash(() => decompileHighLevelBytes(aef), "2000 pushes no pop");
});
...
test("stack-depth: deep DUP chain", () => {
  ...
  script.push(0x40); // RET
  const aef = buildValidAef(new Uint8Array(script));
  mustNotCrash(() => decompileHighLevelBytes(aef), "500 DUPs");
});
...
test("stack-depth: underflow via excessive DROP", () => {
  ...
  script.push(0x40);
  const aef = buildValidAef(new Uint8Array(script));
  mustNotCrash(() => decompileHighLevelBytes(aef), "100 DROPs on empty stack");
});
...
test("stack-depth: underflow via binary ops on empty stack", () => {
  ...
  const aef = buildValidAef(new Uint8Array(script));
  mustNotCrash(() => decompileHighLevelBytes(aef), "binary ops on empty stack");
});
...
test("stack-depth: alternating push/pop 500 times", () => {
  ...
  script.push(0x40);
  const aef = buildValidAef(new Uint8Array(script));
  mustNotCrash(() => decompileHighLevelBytes(aef), "alternating push/pop 500x");
});
...
test("stack-depth: SWAP/ROT/ROLL on nearly empty stack", () => {
  ...
  for (const bytes of scripts) {
    const aef = buildValidAef(new Uint8Array(bytes));
    mustNotCrash(() => decompileHighLevelBytes(aef), "stack ops on shallow stack");
  }
});
...
test("stack-depth: PICK with index exceeding stack", () => {
  ...
  const aef = buildValidAef(new Uint8Array(script));
  mustNotCrash(() => decompileHighLevelBytes(aef), "PICK past stack");
});
...
test("control-flow: deeply nested if/else (50 levels)", () => {
  ...
  script.push(0x40); // RET
  const aef = buildValidAef(new Uint8Array(script));
  mustNotCrash(() => decompileHighLevelBytes(aef), "50-deep nested if");
});
...
test("control-flow: 100 sequential if/else blocks", () => {
  ...
  script.push(0x40);
  const aef = buildValidAef(new Uint8Array(script));
  mustNotCrash(() => decompileHighLevelBytes(aef), "100 sequential if/else");
});
...
test("control-flow: loop within loop (nested backward jumps)", () => {
  ...
  const aef = buildValidAef(new Uint8Array(script));
  mustNotCrash(() => decompileHighLevelBytes(aef), "nested backward jumps");
});
...
test("control-flow: backward jump to self (infinite loop detection)", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "JMP to self");
});
...
test("control-flow: unreachable code blocks", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "unreachable code block");
});
...
test("control-flow: try/catch in a loop", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "try/catch in loop");
});
...
test("control-flow: try with both catch and finally", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "try/catch/finally");
});
...
test("control-flow: nested try blocks (3 levels)", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "3-level nested try");
});
...
test("control-flow: all conditional jump types in sequence", () => {
  ...
  const aef = buildValidAef(new Uint8Array(script));
  mustNotCrash(() => decompileHighLevelBytes(aef), "all conditional jumps");
});
...
test("control-flow: CALL to internal methods", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "CALL to internal method");
});
...
test("control-flow: CALL_L with 32-bit offset", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "CALL_L 32-bit offset");
});
...
test("operand-boundary: PUSHINT8 at min/max (-128, 0, 127)", () => {
  ...
  for (const v of values) {
    ...
    const aef = buildValidAef(script);
    mustNotCrash(() => decompileHighLevelBytes(aef), `PUSHINT8 val=${v}`);
  }
});
...
test("operand-boundary: PUSHINT16 at boundaries", () => {
  ...
  for (const [lo, hi] of values) {
    ...
    const aef = buildValidAef(script);
    mustNotCrash(() => decompileHighLevelBytes(aef), `PUSHINT16 [${lo},${hi}]`);
  }
});
...
test("operand-boundary: PUSHINT32 at boundaries", () => {
  ...
  for (const bytes of values) {
    ...
    const aef = buildValidAef(script);
    mustNotCrash(() => decompileHighLevelBytes(aef), `PUSHINT32 ${bytes}`);
  }
});
...
test("operand-boundary: PUSHINT64 at boundaries", () => {
  ...
  for (const bytes of values) {
    ...
    const aef = buildValidAef(script);
    mustNotCrash(() => decompileHighLevelBytes(aef), `PUSHINT64`);
  }
});
...
test("operand-boundary: PUSHINT128 and PUSHINT256", () => {
  ...
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(script128z)), "PUSHINT128 zeros");

  // All 0xFF
  const script128f = new Uint8Array([0x04, ...new Array(16).fill(0xFF), 0x40]);
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(script128f)), "PUSHINT128 0xFF");

  // PUSHINT256 all zeros
  const script256z = new Uint8Array([0x05, ...new Array(32).fill(0), 0x40]);
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(script256z)), "PUSHINT256 zeros");

  // PUSHINT256 all 0xFF
  const script256f = new Uint8Array([0x05, ...new Array(32).fill(0xFF), 0x40]);
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(script256f)), "PUSHINT256 0xFF");

  // PUSHINT256 max positive
  const script256max = new Uint8Array([0x05, ...new Array(31).fill(0xFF), 0x7F, 0x40]);
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(script256max)), "PUSHINT256 max positive");
});
...
test("operand-boundary: U8 operand 0 and 255", () => {
  ...
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s0)), "U8 operand 0");
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s255)), "U8 operand 255");
});
...
test("operand-boundary: U16 operand 0 and 65535", () => {
  ...
  const aef0 = buildValidAef(s0);
  const aef65535 = buildValidAef(s65535);
  mustNotCrash(() => decompileHighLevelBytes(aef0), "U16 operand 0");
  mustNotCrash(() => decompileHighLevelBytes(aef65535), "U16 operand 65535");
});
...
test("operand-boundary: Jump8 signed range (-128 to 127)", () => {
  for (const val of [-128, -1, 0, 1, 2, 127]) {
    ...
    const aef = buildValidAef(script);
    mustNotCrash(() => decompileHighLevelBytes(aef), `Jump8 val=${val}`);
  }
});
...
test("operand-boundary: Jump32 extreme values", () => {
  ...
  for (const val of [-2147483648, -1, 0, 1, 2147483647]) {
    ...
    const aef = buildValidAef(script);
    mustNotCrash(() => decompileHighLevelBytes(aef), `Jump32 val=${val}`);
  }
});
...
test("operand-boundary: PUSHDATA1 empty and max-size data", () => {
  ...
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(empty)), "PUSHDATA1 empty");

  // 255 bytes of data (max for Data1)
  const maxData = [0x0C, 0xFF, ...new Array(255).fill(0xAA), 0x40];
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(new Uint8Array(maxData))), "PUSHDATA1 255 bytes");
});
...
test("operand-boundary: PUSHDATA2 larger payloads", () => {
  ...
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(new Uint8Array(data256))), "PUSHDATA2 256 bytes");

  // 1000 bytes
  const data1k = [0x0D, 0xE8, 0x03, ...new Array(1000).fill(0xCC), 0x40];
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(new Uint8Array(data1k))), "PUSHDATA2 1000 bytes");
});
...
test("operand-boundary: ISTYPE and CONVERT with all type IDs", () => {
  ...
  for (const typeId of typeIds) {
    ...
    mustNotCrash(
      () => decompileHighLevelBytes(buildValidAef(scriptIsType)),
      `ISTYPE type=${typeId}`,
    );
    const scriptConvert = new Uint8Array([0x11, 0xDB, typeId, 0x40]);
    mustNotCrash(
      () => decompileHighLevelBytes(buildValidAef(scriptConvert)),
      `CONVERT type=${typeId}`,
    );
  }
});
...
test("method-tokens: 0 tokens", () => {
  const aef = buildAef({ tokens: [], script: new Uint8Array([0x11, 0x40]) });
  const result = parseAef(aef);
...
test("method-tokens: 1 token with CALLT 0", () => {
  ...
  const aef = buildAef({ tokens: [token], script });
  mustNotCrash(() => decompileHighLevelBytes(aef), "CALLT with 1 token");
});
...
test("method-tokens: 128 tokens (half of max)", () => {
  ...
  const aef = buildAef({ tokens, script });
  mustNotCrash(() => decompileHighLevelBytes(aef), "128 tokens");
});
...
test("method-tokens: name longer than the 32-byte spec limit is rejected", () => {
  ...
  const script = new Uint8Array([0x37, 0x00, 0x00, 0x40]);
  const aef = buildAef({ tokens: [token], script });
  assert.throws(() => parseAef(aef), (error) => error.details.code === "InvalidMethodToken");
});
...
test("method-tokens: token with 0 params and no return", () => {
  ...
  const script = new Uint8Array([0x37, 0x00, 0x00, 0x40]);
  const aef = buildAef({ tokens: [token], script });
  mustNotCrash(() => decompileHighLevelBytes(aef), "token with 0 params, no return");
});
...
test("method-tokens: token with max params (65535)", () => {
  ...
  const script = new Uint8Array([0x37, 0x00, 0x00, 0x40]);
  const aef = buildAef({ tokens: [token], script });
  mustNotCrash(() => decompileHighLevelBytes(aef), "token with 65535 params");
});
...
test("method-tokens: multiple CALLT references to different tokens", () => {
  ...
  const aef = buildAef({ tokens, script });
  mustNotCrash(() => decompileHighLevelBytes(aef), "multiple CALLT calls");
});
...
test("method-tokens: CALLT index out of bounds", () => {
  ...
  const aef = buildAef({ tokens: [token], script });
  mustNotCrash(() => decompileHighLevelBytes(aef), "CALLT out of bounds");
});
...
test("manifest: valid manifest with multiple methods", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(
    () => decompileHighLevelBytesWithManifest(aef, manifest),
...
test("manifest: empty manifest object", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(
    () => decompileHighLevelBytesWithManifest(aef, "{}"),
    "empty manifest",
  );
});
...
test("manifest: manifest with null fields", () => {
  ...
  const aef = buildValidAef(new Uint8Array([0x11, 0x40]));
  for (const m of manifests) {
    mustNotCrash(
      () => decompileHighLevelBytesWithManifest(aef, m),
      `manifest: ${m}`,
    );
  }
});
...
test("manifest: analyzeBytes with and without manifest", () => {
  const aef = buildValidAef(new Uint8Array([0x11, 0x40]));
  mustNotCrash(() => analyzeBytes(aef), "analyzeBytes without manifest");
  mustNotCrash(
    () => analyzeBytes(aef, '{"name":"Test","abi":{"methods":[]}}'),
...
test("regression: all-zero script", () => {
  ...
  for (const s of scripts) {
    const aef = buildValidAef(s);
    mustNotCrash(() => decompileHighLevelBytes(aef), "all-zero-ish script");
  }
});
...
test("regression: all-0xFF script body", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "all-0xFF script");
});
...
test("regression: script ending mid-operand", () => {
  ...
  for (const script of truncated) {
    ...
    mustNotCrash(
      () => {
        try { decompileBytes(aef); } catch { /* expected */ }
...
test("regression: jump to self variations", () => {
  ...
  for (const s of scripts) {
    const aef = buildValidAef(s);
    mustNotCrash(() => decompileHighLevelBytes(aef), "jump to self variant");
  }
});
...
test("regression: ABORT sequences", () => {
  ...
  for (const s of scripts) {
    const aef = buildValidAef(s);
    mustNotCrash(() => decompileHighLevelBytes(aef), "ABORT sequence");
  }
});
...
test("regression: PUSHA with various target offsets", () => {
  ...
  for (const off of offsets) {
    ...
    const aef = buildValidAef(script);
    mustNotCrash(() => decompileHighLevelBytes(aef), `PUSHA offset=${off}`);
  }
});
...
test("regression: deeply nested PACK/UNPACK", () => {
  ...
  script.push(0x40);
  const aef = buildValidAef(new Uint8Array(script));
  mustNotCrash(() => decompileHighLevelBytes(aef), "nested PACK/UNPACK");
});
...
test("regression: INITSSLOT and static field operations", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "static field operations");
});
...
test("regression: INITSLOT then local and arg operations", () => {
  ...
  const aef = buildValidAef(script);
  const result = decompileHighLevelBytes(aef);
...
test("regression: buffer operations sequence", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "buffer operations");
});
...
test("regression: map operations sequence", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "map operations");
});
...
test("regression: NOP-only script (large)", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "1000 NOPs");
});
...
test("regression: WITHIN, MIN, MAX with stacked args", () => {
  ...
  mustNotCrash(
    () => decompileHighLevelBytes(buildValidAef(scriptWithin)),
    "WITHIN",
  );
  // MIN and MAX need 2
  const scriptMin = new Uint8Array([0x11, 0x12, 0xB9, 0x40]);
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(scriptMin)), "MIN");
  const scriptMax = new Uint8Array([0x11, 0x12, 0xBA, 0x40]);
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(scriptMax)), "MAX");
});

test("regression: MODMUL and MODPOW (3-arg opcodes)", () => {
  const scriptModmul = new Uint8Array([0x11, 0x12, 0x13, 0xA5, 0x40]);
  mustNotCrash(
    () => decompileHighLevelBytes(buildValidAef(scriptModmul)),
    "MODMUL",
  );
  const scriptModpow = new Uint8Array([0x11, 0x12, 0x13, 0xA6, 0x40]);
  mustNotCrash(
    () => decompileHighLevelBytes(buildValidAef(scriptModpow)),
    "MODPOW",
  );
});
...
test("regression: SUBSTR, LEFT, RIGHT string ops", () => {
  ...
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s1)), "SUBSTR");
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s2)), "LEFT");
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s3)), "RIGHT");
});
...
test("regression: MEMCPY (5-arg opcode)", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "MEMCPY");
});
...
test("regression: CONVERT to every standard type", () => {
  ...
  for (const t of stackTypes) {
    ...
    mustNotCrash(
      () => decompileHighLevelBytes(buildValidAef(script)),
...
test("regression: ISNULL on various values", () => {
  ...
  for (const s of scripts) {
    mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s)), "ISNULL");
  }
});
...
test("regression: double RET", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "double RET");
});
...
test("regression: NZ and boolean logic chain", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "NZ + boolean logic");
});
...
test("regression: DEPTH and CLEAR on various stacks", () => {
  ...
  for (const s of scripts) {
    mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s)), "DEPTH/CLEAR");
  }
});
...
test("regression: REVERSE3, REVERSE4, REVERSEN", () => {
  ...
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s3)), "REVERSE3");
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s4)), "REVERSE4");
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(sn)), "REVERSEN");
});
...
test("regression: TUCK and OVER patterns", () => {
  ...
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s1)), "TUCK");
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s2)), "OVER");
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s3)), "NIP");
});

test("regression: XDROP with stacked values", () => {
  const s = new Uint8Array([0x11, 0x12, 0x13, 0x11, 0x48, 0x40]); // 3 values, PUSH1, XDROP
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s)), "XDROP");
});
...
test("regression: PACKMAP and PACKSTRUCT", () => {
  ...
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(sPm)), "PACKMAP");
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(sPs)), "PACKSTRUCT");
});
...
test("regression: POPITEM on array", () => {
  ...
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s)), "POPITEM");
});
...
test("regression: large PUSHDATA4", () => {
  ...
  const aef = buildValidAef(script);
  mustNotCrash(() => decompileHighLevelBytes(aef), "PUSHDATA4 5000 bytes");
});
...
test("regression: EQUAL and NOTEQUAL with mixed types", () => {
  ...
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s1)), "EQUAL mixed 1");
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s2)), "EQUAL mixed 2");
  mustNotCrash(() => decompileHighLevelBytes(buildValidAef(s3)), "NOTEQUAL mixed");
});
