// Spec-fidelity tests for manifest permission descriptors, the 
// `features` contract, and AEF varint/limit behavior. Each case mirrors
// a Rust test (src/manifest/tests.rs, src/aef/tests/) so the two ports
...
// ─── AEF spec limits and varint behavior ───────────────────────────────────
...
function buildRawAef({ script = [0x40], sourceVarInt = [0x00] } = {}) {
  const data = [];
...
test("manifest-spec: AEF non-canonical varint (FD 05 00) parses like the reference", () => {
  ...
  const aef = buildRawAef({ sourceVarInt: [0xfd, 0x05, 0x00, ...sourceBytes] });
  const parsed = parseAef(aef);
  assert.equal(parsed.header.source, "s.com");
});

test("manifest-spec: AEF invalid magic surfaces InvalidMagic even for invalid UTF-8", () => {
  // 0xFF leading byte is not valid UTF-8 — this used to escape as a
  // raw TypeError from TextDecoder instead of a AefParseError.
  const data = buildRawAef();
  data[0] = 0xff;
  assert.throws(() => parseAef(data), (error) => {
