...
test("high-level forward jumps restore and merge stack values", () => {
  ...
  const { highLevel } = decompileHighLevelBytes(buildAef({ script }));
...
/**
 * For each operand encoding, we pick an opcode that uses it and build a AEF
...
for (const [description, opcodeByte, operandSize] of truncationCases) {
  ...
  test(`disassembler truncation: ${description} also throws via disassembleScript`, () => {
    // Test the raw disassembleScript path too (no AEF wrapper needed)
    const truncatedOperand = new Uint8Array(operandSize - 1).fill(0x00);
...
test("inline use-count: temp used twice is NOT inlined", () => {
  ...
  const aef = buildAef({ script });
  const result = decompileHighLevelBytes(aef, { inlineSingleUseTemps: true });
...
test("inline use-count: multi-use temp in expression is preserved (DUP pattern)", () => {
  ...
  const aef = buildAef({ script });
  const resultWithInline = decompileHighLevelBytes(aef, { inlineSingleUseTemps: true });
  const resultWithout = decompileHighLevelBytes(aef, { inlineSingleUseTemps: false });
