...
import { buildAefFromScript } from "./decompiler-fixtures.mjs";

test("rewrites CONVERT using the operand target helper name", () => {
  const script = new Uint8Array([0x11, 0xdb, 0x28, 0x40]);
  const result = decompileHighLevelBytes(buildAefFromScript(script));
...
test("lifts unary math helpers", () => {
  const notResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0xaa, 0x40])),
  );
  const negateResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x9b, 0x40])),
  );
  const absResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x9a, 0x40])),
  );
  const signResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x99, 0x40])),
  );
  const invertResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x90, 0x40])),
  );
...
test("lifts binary math and helper ops", () => {
  const andResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x11, 0x91, 0x40])),
  );
  const orResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x11, 0x92, 0x40])),
  );
  const xorResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x11, 0x93, 0x40])),
  );
  const sqrtResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0xa4, 0x40])),
  );
  const modmulResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x11, 0x11, 0xa5, 0x40])),
  );
  const withinResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x11, 0x11, 0xbb, 0x40])),
  );
  const leftResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x11, 0x8d, 0x40])),
  );
  const rightResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x11, 0x8e, 0x40])),
  );
...
test("lifts SUBSTR as a helper call", () => {
  const result = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x11, 0x11, 0x8c, 0x40])),
  );
  assert.match(result.highLevel, /return substr\(1, 1, 1\);/);
});

test("handles OVER, SWAP, and NIP stack shuffles", () => {
  const over = decompileHighLevelBytes(buildAefFromScript(new Uint8Array([0x11, 0x12, 0x4b, 0x40])));
  const swap = decompileHighLevelBytes(buildAefFromScript(new Uint8Array([0x11, 0x12, 0x50, 0x40])));
  const nip = decompileHighLevelBytes(buildAefFromScript(new Uint8Array([0x11, 0x12, 0x46, 0x40])));
...
test("lifts DEPTH and CLEAR stack ops", () => {
  const depthResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x12, 0x43, 0x40])),
  );
  const clearResult = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x49, 0x40])),
  );
...
test("supports indexed local and argument slots", () => {
  ...
  const local = decompileHighLevelBytes(buildAefFromScript(localScript));
  const arg = decompileHighLevelBytes(buildAefFromScript(argScript));
...
test("rewrites SIZE and ISNULL helpers", () => {
  ...
  const size = decompileHighLevelBytes(buildAefFromScript(sizeScript));
  const isNull = decompileHighLevelBytes(buildAefFromScript(nullScript));
...
test("rewrites NEWARRAY, NEWSTRUCT, and NEWBUFFER helpers", () => {
  const newArray = decompileHighLevelBytes(buildAefFromScript(new Uint8Array([0x11, 0xc3, 0x40])));
  const newStruct = decompileHighLevelBytes(buildAefFromScript(new Uint8Array([0x11, 0xc6, 0x40])));
  const newBuffer = decompileHighLevelBytes(buildAefFromScript(new Uint8Array([0x11, 0x88, 0x40])));
...
test("rewrites NEWARRAY_T using the operand type tag", () => {
  const result = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0xc4, 0x40, 0x40])),
  );
...
test("rewrites REMOVE, CLEARITEMS, and REVERSEITEMS helpers", () => {
  const remove = decompileHighLevelBytes(buildAefFromScript(new Uint8Array([0xc8, 0x10, 0xd2, 0x40])));
  const clear = decompileHighLevelBytes(buildAefFromScript(new Uint8Array([0xc8, 0xd3, 0x40])));
  const reverse = decompileHighLevelBytes(buildAefFromScript(new Uint8Array([0xc2, 0xd1, 0x40])));
...
test("abort and throw clear the tracked stack", () => {
  const abort = decompileHighLevelBytes(buildAefFromScript(new Uint8Array([0x11, 0x38, 0x40])));
  const throwResult = decompileHighLevelBytes(buildAefFromScript(new Uint8Array([0x11, 0x3a, 0x40])));
...
test("DROP followed by THROW preserves a payloadless VM throw", () => {
  const result = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x45, 0x3A, 0x40])),
  );
...
test("abortmsg and assertmsg use their message and condition arguments", () => {
  const abortmsg = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x0c, 0x01, 0x41, 0xe0, 0x40])),
  );
  const assertmsg = decompileHighLevelBytes(
    buildAefFromScript(new Uint8Array([0x11, 0x0c, 0x01, 0x42, 0xe1, 0x40])),
  );
