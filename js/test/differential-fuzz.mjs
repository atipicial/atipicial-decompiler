/**
 * Differential fuzz tester: generates random valid AEF files and compares
...
const RUST_BIN = join(ROOT, "target", "release", "atipicial-decompiler");
const TEMP_DIR = mkdtempSync(join(tmpdir(), "atipicial-diff-fuzz-"));
...
function runRustDisasm(aefPath) {
  try {
    const stdout = execFileSync(RUST_BIN, ["disasm", aefPath], {
      encoding: "utf-8",
      timeout: 10_000,
...
function runRustDecompile(aefPath) {
  try {
    ...
    const stdout = execFileSync(RUST_BIN, ["decompile", "--format", "high-level", aefPath], {
      encoding: "utf-8",
      timeout: 10_000,
      stdio: ["pipe", "pipe", "pipe"],
    });
...
for (const ec of edgeCases) {
  edgeStats.total++;
  const aef = buildAefFromScript(new Uint8Array(ec.script));
  const path = join(TEMP_DIR, `edge_${ec.name}.aef`);
  writeFileSync(path, aef);

  const rustDisasm = runRustDisasm(path);
  let jsOk = false;
  let jsResult = null;
  try {
    jsResult = decompileBytes(aef);
    jsOk = true;
  } catch {}
...
console.log(`Differential fuzz: generating ${NUM_CASES} random AEF files in ${TEMP_DIR}`);
...
for (let i = 0; i < NUM_CASES; i++) {
  ...
  const aefBytes = buildAefFromScript(script);
  const aefPath = join(TEMP_DIR, `fuzz_${i.toString().padStart(4, "0")}.aef`);
  writeFileSync(aefPath, aefBytes);

  // ---- Disassembly comparison ----
  const rustDisasm = runRustDisasm(aefPath);

  let jsDisasmOk = false;
  let jsResult = null;
  try {
    jsResult = decompileBytes(aefBytes);
    jsDisasmOk = true;
  } catch {
    jsDisasmOk = false;
  }
  ...
  const rustDecompile = runRustDecompile(aefPath);
  ...
  try {
    jsHighResult = decompileHighLevelBytes(aefBytes, { clean: true });
    jsHighOk = true;
  } catch {
    jsHighOk = false;
  }
