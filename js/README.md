...
# atipicial-decompiler-js

Pure JavaScript Atipicial smart contract decompiler. Parse AEF files, disassemble bytecode, and decompile to human-readable pseudocode. Zero dependencies.

## Install

```bash
npm install atipicial-decompiler-js
```

## Features

- **AEF parsing** — validate magic, checksum, method tokens
- **Disassembly** — full Atipicial VM opcode coverage
...
import {
  ...
  analyzeBytes,
  parseManifest,
} from "atipicial-decompiler-js";

// Basic: parse and decompile
const result = decompileHighLevelBytes(aefBytes);
console.log(result.highLevel);

// With manifest for better output
const manifest = parseManifest(manifestJson);
const withManifest = decompileHighLevelBytesWithManifest(aefBytes, manifest);
console.log(withManifest.highLevel);

// Full analysis: call graph, method contracts, xrefs, types
const analysis = analyzeBytes(aefBytes, manifestJson);
...
// Step by step
const aef = parseAef(aefBytes);
const disasm = disassembleScript(aef.script);
console.log(disasm.instructions);
```

## API

### `parseAef(bytes) → { script, header, methodTokens, scriptHash, scriptHashLE, ... }`

Parse a AEF container. Throws on invalid magic or checksum mismatch. Returns the script hash in both big-endian (`scriptHash`) and little-endian (`scriptHashLE`) hex.

### `disassembleScript(script) → { instructions, warnings }`

Disassemble a bytecode array into instruction objects.

### `decompileBytes(bytes) → { aef, instructions, warnings, pseudocode }`
...
source output is intentionally focused on readable Atipicial C# contracts; the target
hint is secondary analysis only. Manifest standards are high confidence; bytecode-only hints remain conservative. ABI
method signatures can infer AEP-17, AEP-11, and AEP-24 (`royaltyInfo`) when
the manifest does not declare a standard.

When the pinned v3.10.0 corpus and Atipicial framework assembly are available, the
optional Roslyn gate verifies every generated JS contract:

```bash
ATC_CSHARP_CORPUS_DIR=/path/to/devpack-artifacts-v3.10.0 \
ATC_SMARTCONTRACT_FRAMEWORK_DLL=/path/to/Atipicial.SmartContract.Framework.dll \
ATC_CSHARP_TARGET_FRAMEWORK=net10.0 \
...
Parse a Atipicial contract manifest JSON.
...
| atipicial-decompiler-js | atipicial-decompiler (Rust) |
|-------------------|-----------------------|
| 2.0.0             | 0.12.0                |
| 1.6.0             | 0.11.0                |
| 1.5.4             | 0.10.2                |
