import { AefParseError } from "./errors.js";
import {
  asUint8Array,
  computeChecksum,
  computeScriptHash,
...
const MAX_AEF_FILE_SIZE = 0x10_0000;
const FIXED_HEADER_SIZE = 68;
const CHECKSUM_SIZE = 4;
const MAGIC = "AEF3";
// 'AEF3' interpreted as a little-endian uint32, per the reference
// `AefFile.Magic` definition.
const MAGIC_U32 = 0x3346454e;
const MAX_SOURCE_LEN = 256;
// Reference: `AefFile.Deserialize` reads tokens with
...
export function parseAef(input) {
  const bytes = asUint8Array(input);
  if (bytes.byteLength > MAX_AEF_FILE_SIZE) {
    throw new AefParseError(
      `file size ${bytes.byteLength} exceeds maximum (${MAX_AEF_FILE_SIZE} bytes)`,
      { code: "FileTooLarge", size: bytes.byteLength, max: MAX_AEF_FILE_SIZE },
    );
  }
  if (bytes.byteLength < FIXED_HEADER_SIZE + CHECKSUM_SIZE) {
    throw new AefParseError("file too short to contain a AEF header", {
      code: "TooShort",
    });
  }
  ...
  try {
    ...
    throw new AefParseError("compiler field is not valid UTF-8", {
  ...
  if (script.byteLength === 0) {
    throw new AefParseError("script section cannot be empty", {
      code: "EmptyScript",
    });
  }
  ...
  return {
    ...
    script,
    checksum,
    // The raw RIPEMD160(SHA256(..)) digest IS Atipicial's internal little-endian
...
function parseMethodTokens(bytes, startOffset) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (methodLen > MAX_METHOD_NAME_LEN) {
      throw new AefParseError(`invalid method token at index ${index}`, {
    ...
    if (method.startsWith("_")) {
      throw new AefParseError(`method token name ${JSON.stringify(method)} is not permitted`, {
...
function readVarInt(bytes, offset) {
  ...
  if (first <= 0xfc) {
    ...
    if (wide > BigInt(0xffffffff)) {
      throw new AefParseError(`varint exceeds supported range at offset ${offset}`, {
  ...
  // encodings (e.g. `FD 05 00` for 5) are accepted, because AEF
...
function readVarString(bytes, offset, maxLength) {
  ...
  if (length > maxLength) {
    throw new AefParseError(`source string exceeds maximum length (${length} > ${maxLength})`, {
      code: "SourceTooLong",
      length,
      max: maxLength,
    });
  ...
  try {
    ...
    throw new AefParseError(`varstring contains invalid utf-8 at offset ${start}`, {
...
function readVarBytes(bytes, offset, maxLength) {
  const { value: length, consumed } = readVarInt(bytes, offset);
  if (length > maxLength) {
    throw new AefParseError(`script exceeds maximum size (${length} > ${maxLength})`, {
      code: "ScriptTooLarge",
      length,
      max: maxLength,
    });
...
function unexpectedEof(offset) {
  return new AefParseError(`unexpected end of data at offset ${offset}`, {
    code: "UnexpectedEof",
    offset,
  });
}
