// Type declarations for atipicial-decompiler-js
...
export interface AefHeader {
  magic: string;
  compiler: string;
  source: string;
}

export interface AefFile {
  script: Uint8Array;
  header: AefHeader;
  ...
  scriptHash: string;
  /** Little-endian (Atipicial internal `UInt160`) form of the script hash — the
...
/**
 * Raw `features` object from the manifest. Atipicial requires this to be
 * an empty object (the legacy 2.x `storage`/`payable` flags do not
 * exist in ); tolerant parsing surfaces whatever a malformed
...
/** CALLT edge whose token index resolved inside `aef.methodTokens`. */
export interface MethodTokenCallTarget {
  kind: "MethodToken";
  /** Index into `aef.methodTokens` (the CALLT operand). */
  index: number;
...
export function parseAef(bytes: Uint8Array | ArrayBuffer | number[]): AefFile;
