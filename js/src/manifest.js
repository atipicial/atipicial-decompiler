...
function parseFeatures(features) {
  ...
  // Atipicial's `ContractManifest.FromJson` requires `features` to be an
  // empty object (the legacy 2.x storage/payable flags do not exist in
  // ). Tolerant parsing keeps the raw object so malformed manifests
...
function validateManifestStrict(manifest) {
  if (Object.keys(manifest.features).length > 0) {
    throw new ManifestParseError(
      `features must be an empty object in Atipicial, got keys: ${Object.keys(manifest.features).join(", ")}`,
