 ...
 */
export function identifyPatterns(aef, instructions, manifest = null) {
  ...
  for (const standard of declaredStandards) {
    ...
    if (normalized.startsWith("AEP-")) patterns.add(normalized);
  }
  ...
  if ((aef?.methodTokens ?? []).length > 0) {
    patterns.add("method_tokens");
    evidence.push({ source: "aef.method_tokens", value: String(aef.methodTokens.length) });
  }
  for (const token of aef?.methodTokens ?? []) {
    ...
    if (hint?.hasExactMethod()) {
      ...
      evidence.push({
        source: "aef.method_tokens.native",
        value: label,
      });
  ...
  const compiler = aef?.header?.compiler?.trim() || null;
  if (compiler) evidence.push({ source: "aef.header.compiler", value: compiler });
  if (aef?.header?.source?.trim()) {
    evidence.push({ source: "aef.header.source", value: aef.header.source });
  }
  const compilerLanguage = inferLanguage(compiler);
  const language = compilerLanguage ?? inferLanguageFromSource(aef?.header?.source);
  const strongInferredPattern = patterns.has("AEP-17") || patterns.has("AEP-11");
...
function inferLanguage(compiler) {
  ...
  // Fixed-width AEF compiler tags may be short (`cs`, `cs__`) rather than full names.
