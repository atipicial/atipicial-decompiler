...
export function buildHighLevelContext(
  ...
  const context = {
    ...
    // known calls render as `AtipicialDollar::Transfer(...)` instead of only the
    // raw method name. Unknown or restricted tokens retain their raw label.
    calltLabels: aef.methodTokens.map((token) => {
    ...
    calltParamCounts: aef.methodTokens.map((token) => token.parametersCount),
    calltReturnsValue: aef.methodTokens.map((token) => token.hasReturnValue),
    methodTokens: aef.methodTokens,
    scriptHash: aef.scriptHash,
    scriptHashLE: aef.scriptHashLE,
    compiler: aef.header?.compiler,
    source: aef.header?.source,
    highLevelWarnings: [],
    postprocessOptions: {
