...
export function renderCSharpContract(
  ...
  const output = [
    ...
    "using Atipicial.SmartContract.Framework.Native;",
  ...
  for (const [lineIndex, line] of sourceLines.entries()) {
    ...
    if (nonVoidMethodInfo.bodyLines.has(lineIndex) && line.trim() === "return;") {
      const indentation = line.match(/^\s*/)?.[0] ?? "";
      output.push(`${indentation}throw new InvalidOperationException("Unreachable Atipicial VM fallthrough.");`);
      continue;
    }
  ...
  if (!classSeen) {
    output.push("public class AtipicialContract : SmartContract {");
    output.push(...patternComments);
