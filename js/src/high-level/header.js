...
import { describeCallFlags } from "../aef.js";
...
export function renderContractHeader(manifest, context = null) {
  ...
  if (!manifest) {
    ...
    // tokens declared in AEF`, while Rust runs them flush.
  ...
  if (manifest) {
    ...
    if (manifest.permissions?.length) {
      ...
      for (const perm of manifest.permissions) {
        ...
        // An absent `methods` defaults to the `*` wildcard in Atipicial (Rust's
