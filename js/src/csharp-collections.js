...
export function createCSharpCollectionHelpers(rewriteExpression) {
  return new Map([
    ...
    // Box the operand before the null test so literals and value-like Atipicial
    ...
    ["clear_items", (args, types) => {
      ...
      return kind === "map"
        ? `${args[0]}.Clear()`
        : kind === "list"
          ? `((Atipicial.SmartContract.Framework.List<${listElementType(args[0], types)}>)${args[0]}).Clear()`
    ...
    ["remove_item", (args, types) => {
      ...
      return kind === "map"
        ...
        : kind === "list"
          ? `((Atipicial.SmartContract.Framework.List<${listElementType(args[0], types)}>)${args[0]}).RemoveAt((int)(${args[1]}))`
          : `((dynamic)${args[0]}).Remove(${args[1]})`;
    }],
    ["append", (args, types) => {
      if (args.length !== 2) return null;
      return collectionKind(args[0], types) === "list"
        ? `((Atipicial.SmartContract.Framework.List<${listElementType(args[0], types)}>)${args[0]}).Add(${args[1]})`
    ...
    ["pop_item", (args, types) => {
      if (args.length !== 1) return null;
      return collectionKind(args[0], types) === "list"
        ? `((Atipicial.SmartContract.Framework.List<${listElementType(args[0], types)}>)${args[0]}).PopItem()`
