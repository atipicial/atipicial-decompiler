...
import { AtipicialDecompilerError, AefParseError, DisassemblyError, ManifestParseError } from "./errors.js";
import { parseAef } from "./aef.js";
...
/**
 * Parse a AEF blob and return its instruction stream plus rendered
 * pseudocode (no manifest correlation, no high-level lifting).
 *
 * @param {Uint8Array | ArrayBuffer | number[]} bytes - Raw AEF bytes.
...
export function decompileBytes(bytes, options = {}) {
  const aef = parseAef(bytes);
  const disassembly = disassembleScript(aef.script, options);
  return {
    aef,
    instructions: disassembly.instructions,
    warnings: disassembly.warnings,
    pseudocode: renderPseudocode(disassembly.instructions),
    patterns: identifyPatterns(aef, disassembly.instructions, null),
  };
}

/**
 * Run the full analysis pipeline (CFG, xrefs, type inference) against
 * a AEF and an optional manifest. Returns the same fields as
...
export function analyzeBytes(bytes, manifestInput = null, options = {}) {
  ...
  const callGraph = buildCallGraph(result.aef, result.instructions, analysisGroups);
  ...
  return {
    ...
    patterns: identifyPatterns(result.aef, result.instructions, manifest),
...
/**
 * Decompile a AEF directly to high-level pseudocode (no manifest).
...
export function decompileHighLevelBytes(bytes, options = {}) {
  ...
  return {
    ...
    csharp: renderCSharpContract(
      highLevel,
      null,
      { ...options, methodTokens: result.aef.methodTokens },
      patterns,
    ),
  };
}
...
export function decompileHighLevelBytesWithManifest(bytes, manifestInput, options = {}) {
  ...
  const patterns = identifyPatterns(result.aef, result.instructions, manifest);
  return {
    ...result,
    warnings: [...result.warnings, ...context.highLevelWarnings],
    manifest,
