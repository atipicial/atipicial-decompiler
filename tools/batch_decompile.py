#!/usr/bin/env python3
"""
Batch-decompile all extracted devpack AEF+manifest pairs and collect results.
...
BINARY = REPO_ROOT / "target" / "release" / "atipicial-decompiler"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=BINARY,
        help=f"Path to atipicial-decompiler binary (default: {BINARY})",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> None:
    ...
    aef_files = sorted(DEVPACK_DIR.glob("*.aef"))
    print(f"Decompiling {len(aef_files)} contracts with {args.jobs} worker(s)...")
    ...
    for aef_path in aef_files:
        ...
        if not manifest_path.exists():
            ...
            continue
        planned.append((name, aef_path, manifest_path))
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.jobs) as executor:
        future_to_name = {
            ...
            for name, aef_path, manifest_path in planned
        }
