#!/usr/bin/env python3
"""
Regenerate src/opcodes_generated.rs from Atipicial project's OpCode definition.

Usage:
    python tools/generate_opcodes.py

The script expects tools/OpCode.cs to be present (downloaded from
https://github.com/atipicial-project/atipicial). The generated Rust source is written to
...
LOCAL_OPCODE_SOURCE = REPO_ROOT / "atipicial_csharp" / "vm" / "src" / "Atipicial.VM" / "OpCode.cs"
OUTPUT = REPO_ROOT / "src" / "opcodes_generated.rs"


def to_rust_name(name: str) -> str:
    """Convert ATC opcode identifier to a CamelCase Rust variant name."""
    special_prefix = {
