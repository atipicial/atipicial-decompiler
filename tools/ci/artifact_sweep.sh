...
BIN_PATH="${BIN_PATH:-$ROOT_DIR/target/debug/atipicial-decompiler}"
...
hash_decompiled_outputs() {
    ...
    if [[ -d "$DECOMPILED_DIR" ]]; then
        while IFS= read -r -d '' file; do
            ...
            \( -name '*.high-level.cs' -o -name '*.csharp.cs' -o -name '*.pseudocode.txt' -o -name '*.error.txt' -o -name '*.aef' -o -name '*.manifest.json' \) \
            -print0 | sort -z)
    fi
...
if [[ -n "${ATC_SMARTCONTRACT_FRAMEWORK_DLL:-}" ]]; then
    log "Compiling representative generated C# with Roslyn"
    tools/ci/csharp_compile.sh
else
    log "Skipping Roslyn C# compilation (ATC_SMARTCONTRACT_FRAMEWORK_DLL is unset)"
fi
...
while IFS= read -r -d '' aef_path; do
    manifest_path="${aef_path%.aef}.manifest.json"
    if [[ -f "$manifest_path" ]]; then
        id="${aef_path#$ARTIFACTS_DIR/}"
        id="${id%.aef}"
        add_artifact_pair "$id" "$aef_path" "$manifest_path"
    fi
done < <(find "$ARTIFACTS_DIR" -type f -name '*.aef' ! -path "$DECOMPILED_DIR/*" -print0 | sort -z)

if [[ -d "$DECOMPILED_DIR" ]]; then
    while IFS= read -r -d '' aef_path; do
        manifest_path="${aef_path%.aef}.manifest.json"
        if [[ -f "$manifest_path" ]]; then
            id="${aef_path#$DECOMPILED_DIR/}"
            id="${id%.aef}"
            add_artifact_pair "$id" "$aef_path" "$manifest_path"
        fi
    done < <(find "$DECOMPILED_DIR" -type f -name '*.aef' -print0 | sort -z)
fi
...
for id in "${!ARTIFACT_PATHS[@]}"; do
    ...
    if "$BIN_PATH" --manifest "$manifest_path" info --format json "$aef_path" >"$info_out" 2>"$info_err"; then
        if [[ -n "$invalid_key" ]]; then
            pair_failed=1
    ...
    if "$BIN_PATH" disasm --format json "$aef_path" >"$disasm_out" 2>"$disasm_err"; then
        if [[ -n "$invalid_key" ]]; then
            pair_failed=1
    ...
    if "$BIN_PATH" --manifest "$manifest_path" decompile --format json "$aef_path" >"$decompile_out" 2>"$decompile_err"; then
        if [[ -n "$invalid_key" ]]; then
            pair_failed=1
    ...
    if "$BIN_PATH" tokens --format json "$aef_path" >"$tokens_out" 2>"$tokens_err"; then
        if [[ -n "$invalid_key" ]]; then
            pair_failed=1
