#!/usr/bin/env python3
"""Extract reproducible AEF/manifest pairs from atipicial-devpack-dotnet artifacts."""
...
DEFAULT_DEVPACK_ROOT = REPO_ROOT.parent / "atipicial-devpack-dotnet"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "TestingArtifacts" / "devpack"
DEFAULT_EXPECTED_COUNT = 103
UPSTREAM_REPOSITORY = "https://github.com/atipicial-project/atipicial-devpack-dotnet"

AEF_PATTERN = re.compile(
...
class ArtifactExtractionError(ValueError):
    """An artifact source did not contain a valid AEF/manifest pair."""
...
def extract_artifact(cs_path: Path, contracts_src: Path) -> ExtractedArtifact:
    ...
    aef_match = AEF_PATTERN.search(source)
    if not aef_match:
        raise ArtifactExtractionError("embedded AEF was not found")
    ...
    try:
        aef_bytes = base64.b64decode(aef_match.group(1), validate=True)
    except (ValueError, binascii.Error) as error:
        raise ArtifactExtractionError(f"embedded AEF is not valid base64: {error}") from error
...
def existing_generated_names(output_dir: Path) -> tuple[set[str], set[str], set[str]]:
    aef_names = {path.stem for path in output_dir.glob("*.aef")}
    manifest_names = {
    ...
    return aef_names, manifest_names, source_names
...
def write_artifacts(
    ...
    *,
    ...
    for artifact in artifacts:
        (output_dir / f"{artifact.name}.aef").write_bytes(artifact.aef_bytes)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"atipicial-devpack-dotnet checkout (default: {DEFAULT_DEVPACK_ROOT})",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def run(args: argparse.Namespace) -> int:
    ...
    artifacts_src = (
        args.devpack_root
        / "tests"
        / "Atipicial.Compiler.CSharp.UnitTests"
        / "TestingArtifacts"
    )
    contracts_src = args.devpack_root / "tests" / "Atipicial.Compiler.CSharp.TestContracts"
    output_dir = args.output_dir

    if not artifacts_src.is_dir():
        print(f"ERROR: devpack artifacts not found at {artifacts_src}", file=sys.stderr)
        print(
            "Clone atipicial-devpack-dotnet or pass its location with --devpack-root.",
            file=sys.stderr,
        )
        return 1
    ...
    if args.strict and not args.clean and output_dir.exists():
        existing_aefs, existing_manifests, existing_sources = existing_generated_names(
            output_dir
        )
        stale = sorted((existing_aefs | existing_manifests) - expected_names)
        if stale:
