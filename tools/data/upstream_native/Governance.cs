...
//
// Governance.cs file belongs to the atipicial project and is free
...
using Atipicial.Extensions.IO;
using Atipicial.Network.P2P.Payloads;
using Atipicial.Persistence;
using Atipicial.SmartContract.Iterators;
using Atipicial.VM;
...
{
    public const string AtipicialCoinName = "AtipicialCoin";
    public const string AtipicialCoinSymbol = "ATC";
    public const byte AtipicialCoinDecimals = 0;
    public static readonly BigInteger AtipicialCoinFactor = BigInteger.Pow(10, AtipicialCoinDecimals);
    public static readonly BigInteger AtipicialCoinTotalAmount = 100000000 * AtipicialCoinFactor;

    public const string AtipicialDollarName = "AtipicialDollar";
    public const string AtipicialDollarSymbol = "GAS";
    public const byte AtipicialDollarDecimals = 8;
    public static readonly BigInteger AtipicialDollarFactor = BigInteger.Pow(10, AtipicialDollarDecimals);

    public UInt160 AtipicialCoinId => field ??= TokenManagement.GetAssetId(Hash, AtipicialCoinName);
    public UInt160 AtipicialDollarId => field ??= TokenManagement.GetAssetId(Hash, AtipicialDollarName);

    public const decimal EffectiveVoterTurnout = 0.2M;
    private const long VoteFactor = 100000000L;

    private const byte Prefix_AtipicialAccount = 10;
    ...
    private const byte AtipicialHolderRewardRatio = 10;
    ...
    internal override async ContractTask InitializeAsync(ApplicationEngine engine, Hardfork? hardFork)
    {
        if (hardFork == ActiveIn)
        {
            ...
            UInt160 atipicialTokenId = TokenManagement.CreateInternal(engine, Hash, AtipicialCoinName, AtipicialCoinSymbol, AtipicialCoinDecimals, AtipicialCoinTotalAmount);
            await TokenManagement.MintInternal(engine, atipicialTokenId, initialAccount, AtipicialCoinTotalAmount, assertOwner: false, callOnBalanceChanged: false, callOnPayment: false, callOnTransfer: false);
            _OnBalanceChanged(engine, atipicialTokenId, initialAccount, AtipicialCoinTotalAmount, BigInteger.Zero, AtipicialCoinTotalAmount);

            UInt160 gasTokenId = TokenManagement.CreateInternal(engine, Hash, AtipicialDollarName, AtipicialDollarSymbol, AtipicialDollarDecimals, BigInteger.MinusOne);
            ...
            engine.SnapshotCache.Add(CreateStorageKey(Prefix_GasPerBlock, 0u), 5 * AtipicialDollarFactor);
            engine.SnapshotCache.Add(CreateStorageKey(Prefix_RegisterPrice), 1000 * AtipicialDollarFactor);
        }
    }
    ...
    internal override async ContractTask OnPersistAsync(ApplicationEngine engine)
    {
        ...
        {
            await TokenManagement.BurnInternal(engine, AtipicialDollarId, tx.Sender, tx.SystemFee + tx.NetworkFee, assertOwner: false, callOnBalanceChanged: false, callOnTransfer: false);
        ...
        await TokenManagement.MintInternal(engine, AtipicialDollarId, primary, totalNetworkFee, assertOwner: false, callOnBalanceChanged: false, callOnPayment: false, callOnTransfer: false);
    }
    ...
    internal override async ContractTask PostPersistAsync(ApplicationEngine engine)
    {
        ...
        await TokenManagement.MintInternal(engine, Governance.AtipicialDollarId, account, gasPerBlock * CommitteeRewardRatio / 100, assertOwner: false, callOnBalanceChanged: false, callOnPayment: false, callOnTransfer: false);
        ...
        if (ShouldRefreshCommittee(engine.PersistingBlock.Index, m))
        {
            ...
            for (index = 0; index < committee.Count; index++)
            {
                ...
                if (votes > 0)
                {
                    BigInteger voterSumRewardPerATC = factor * voterRewardOfEachCommittee / votes;
                    StorageKey voterRewardKey = CreateStorageKey(Prefix_VoterRewardPerCommittee, publicKey);
                    StorageItem lastRewardPerAtipicial = engine.SnapshotCache.GetAndChange(voterRewardKey, () => new StorageItem(BigInteger.Zero));
                    lastRewardPerAtipicial.Add(voterSumRewardPerATC);
                }
            }
        }
    }
    ...
    {
        if (gasPerBlock < 0 || gasPerBlock > 10 * AtipicialDollarFactor)
            throw new ArgumentOutOfRangeException(nameof(gasPerBlock), $"GasPerBlock must be between [0, {10 * AtipicialDollarFactor}]");
    ...
    public BigInteger UnclaimedGas(ApplicationEngine engine, UInt160 account, uint end)
    {
        ...
        BigInteger balance = TokenManagement.BalanceOf(engine.SnapshotCache, AtipicialCoinId, account);
        if (balance.IsZero) return BigInteger.Zero;
        StorageKey accountKey = CreateStorageKey(Prefix_AtipicialAccount, account);
        AtipicialAccountState state = engine.SnapshotCache[accountKey].GetInteroperable<AtipicialAccountState>();
    ...
    internal async ContractTask<bool> VoteInternal(ApplicationEngine engine, UInt160 account, ECPoint? voteTo)
    {
        ...
        AtipicialAccountState stateAccount = engine.SnapshotCache.GetAndChange(CreateStorageKey(Prefix_AtipicialAccount, account))!.GetInteroperable<AtipicialAccountState>();
        ...
        if (gasDistribution is not null)
            await TokenManagement.MintInternal(engine, Governance.AtipicialDollarId, gasDistribution.Account, gasDistribution.Amount, assertOwner: false, callOnBalanceChanged: false, callOnPayment: true, callOnTransfer: false);
        return true;
    }
    ...
    {
        StorageKey key = CreateStorageKey(Prefix_AtipicialAccount, account);
        return snapshot.TryGet(key)?.GetInteroperable<AtipicialAccountState>().VoteTo;
    }
    ...
    {
        if (assetId != AtipicialCoinId) return;
        if (amount.IsZero) return;
        StorageKey accountKey = CreateStorageKey(Prefix_AtipicialAccount, account);
        AtipicialAccountState accountState = engine.SnapshotCache.GetAndChange(accountKey, () => new(new AtipicialAccountState())).GetInteroperable<AtipicialAccountState>();
    ...
    async ContractTask _OnTransfer(ApplicationEngine engine, UInt160 assetId, UInt160 from, UInt160 to, BigInteger amount, StackItem data)
    {
        ...
        if (amount.IsZero || from == to)
        {
            ...
            // This allows claiming unclaimed gas by transferring 0 ATC
            StorageKey key = CreateStorageKey(Prefix_AtipicialAccount, from);
            var accountState = engine.SnapshotCache.GetAndChange(key)?.GetInteroperable<AtipicialAccountState>();
            if (accountState is null) return;
            BigInteger balance = NativeContract.TokenManagement.BalanceOf(engine.SnapshotCache, AtipicialCoinId, from);
            GasDistribution? distribution = DistributeGas(engine, from, accountState, balance);
            if (distribution is not null)
                await TokenManagement.MintInternal(engine, AtipicialDollarId, distribution.Account, distribution.Amount, assertOwner: false, callOnBalanceChanged: false, callOnPayment: true, callOnTransfer: false);
        }
        else
        {
            var list = engine.CurrentContext!.GetState<ExecutionContextState>().CallingContext!.GetState<List<GasDistribution>>();
    ...
    GasDistribution? DistributeGas(ApplicationEngine engine, UInt160 account, AtipicialAccountState state, BigInteger balance)
    {
    ...
    BigInteger CalculateBonus(IReadOnlyStore snapshot, AtipicialAccountState state, BigInteger balance, uint end)
    {
        if (balance.IsZero) return BigInteger.Zero;
        if (state.BalanceHeight >= end) return BigInteger.Zero;
        var (atipicialHolderReward, voteReward) = CalculateReward(snapshot, state, balance, end);
        return atipicialHolderReward + voteReward;
    }

    (BigInteger AtipicialHolderReward, BigInteger VoteReward) CalculateReward(IReadOnlyStore snapshot, AtipicialAccountState state, BigInteger balance, uint end)
    {
        // Compute Atipicial holder reward
        ...
        return (balance * sumGasPerBlock * AtipicialHolderRewardRatio / 100 / AtipicialCoinTotalAmount, voteReward);
    }
    ...
    IEnumerable<(ECPoint PublicKey, BigInteger Votes)> ComputeCommitteeMembers(IReadOnlyStore snapshot, ProtocolSettings settings)
    {
        ...
        decimal voterTurnout = votersCount / (decimal)AtipicialCoinTotalAmount;
    ...
    class AtipicialAccountState : IInteroperable
    {
