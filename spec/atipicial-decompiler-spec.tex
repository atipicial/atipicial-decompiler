...
\title{Atipicial Decompiler Specification}
...
\texttt{atipicial-decompiler} tool and library. It applies to the following deliverables:
\begin{itemize}[nosep]
    \item The Rust library \texttt{atipicial\_decompiler}.
    \item The CLI binary \texttt{atipicial-decompiler}.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{AEF} refers to a Atipicial  Executable Format blob.
...
\begin{itemize}[nosep]
    \item \textbf{AEF input:} A valid Atipicial  AEF file. The parser shall reject
    files with incorrect magic, checksum, or malformed fields.
    \item \textbf{Manifest input (optional):} A JSON manifest matching the AEF
...
All outputs shall be deterministic with respect to the AEF, manifest, and
...
Field & Description \\
\midrule
file & Path to the AEF file consumed.\\
...
\begin{tabular}{|c|c|c|}
\hline
AEF \& optional manifest & Disassembler \& metadata tables & Views \\
\hline
...
\begin{itemize}[nosep]
    \item Implementations shall include automated tests covering AEF parsing,
