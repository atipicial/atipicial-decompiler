...
const aefFileInput = document.querySelector("#aef-file");
...
async function boot() {
  try {
    ...
    initPanicHook();
    wasmReady = true;
    status.textContent = "Wasm package loaded. Choose a .aef file to begin.";
  } catch (error) {
    wasmReady = false;
    status.innerHTML =
      "Could not load <code>./pkg/atipicial_decompiler.js</code>. Run <code>npm run build:wasm</code> first.";
    console.error(error);
  }
}
...
controls.addEventListener("submit", async (event) => {
  ...
  const aefFile = aefFileInput.files?.[0];
  if (!aefFile) {
    status.textContent = "Choose a .aef file first.";
    return;
  }
  ...
  try {
    const aefBytes = new Uint8Array(await aefFile.arrayBuffer());
