...
export interface WasmBindings {
  infoReport(aefBytes: Uint8Array, options?: unknown): WebInfoReport;
  disasmReport(aefBytes: Uint8Array, options?: unknown): WebDisasmReport;
  decompileReport(aefBytes: Uint8Array, options?: unknown): WebDecompileReport;
  initPanicHook(): void;
}
...
export interface AtipicialDecompilerClient {
  infoReport(aefBytes: Uint8Array, options?: InfoOptions): WebInfoReport;
  disasmReport(aefBytes: Uint8Array, options?: DisasmOptions): WebDisasmReport;
  decompileReport(
    aefBytes: Uint8Array,
...
export function createAtipicialDecompilerClient(
  bindings: WasmBindings,
): AtipicialDecompilerClient {
  return {
    infoReport(aefBytes, options) {
      return bindings.infoReport(aefBytes, normalizeInfoOptions(options));
    },
    disasmReport(aefBytes, options) {
      return bindings.disasmReport(aefBytes, normalizeDisasmOptions(options));
    },
    decompileReport(aefBytes, options) {
...
let defaultClient: AtipicialDecompilerClient | null = null;
const WASM_MODULE_PATH = "./pkg/atipicial_decompiler.js";

export async function init(
  input?: WasmInitInput,
): Promise<AtipicialDecompilerClient> {
  const wasm = (await import(WASM_MODULE_PATH)) as WasmModule;
  await wasm.default(input);
  defaultClient = createAtipicialDecompilerClient({
...
export function infoReport(
  ...
  return requireClient().infoReport(aefBytes, options);
}
...
export function disasmReport(
  ...
  return requireClient().disasmReport(aefBytes, options);
}
...
export function decompileReport(
  ...
  return requireClient().decompileReport(aefBytes, options);
}
...
function requireClient(): AtipicialDecompilerClient {
  if (defaultClient === null) {
    throw new Error(
      "atipicial-decompiler-web is not initialized; call init() before using report functions",
    );
  }
  return defaultClient;
}
