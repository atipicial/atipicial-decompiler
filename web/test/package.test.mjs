...
import { createAtipicialDecompilerClient } from "../dist/index.js";

test("client maps camelCase info options to the wasm ABI", () => {
  let receivedOptions;
  const client = createAtipicialDecompilerClient({
    infoReport(_aefBytes, options) {
  ...
  const aefBytes = new Uint8Array([0xde, 0xad, 0xbe, 0xef]);
  const result = client.infoReport(aefBytes, {
...
test("client maps decompile options to the wasm ABI", () => {
  ...
  const client = createAtipicialDecompilerClient({
    ...
    decompileReport(_aefBytes, options) {
      receivedOptions = options;
      return {
        script_hash_le: "",
        script_hash_be: "",
