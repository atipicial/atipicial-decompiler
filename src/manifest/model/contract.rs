...
/// Representation of a Atipicial contract manifest (`.manifest.json`).
...
pub struct ContractManifest {
    ...
    ///
    /// Atipicial's `ContractManifest.FromJson` requires this to be an empty
    /// JSON object (the legacy 2.x `storage`/`payable` flags do not exist in
    /// ). The raw map is kept so tolerant parsing can surface whatever a
    /// malformed manifest declared; strict parsing rejects non-empty objects.
    #[serde(default)]
    pub features: serde_json::Map<String, Value>,
    /// Supported standards declared by the contract (e.g. `AEP-17`).
