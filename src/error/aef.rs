...
/// Errors returned while parsing AEF containers.
#[derive(Debug, Error)]
#[non_exhaustive]
pub enum AefError {
    /// File too short to contain a AEF header and checksum.
    #[error("file too short to contain a AEF header")]
    TooShort,

    /// Magic bytes at the start of the file did not match the AEF format.
    ...
    ChecksumMismatch {
        /// Checksum stored in the AEF file.
        expected: u32,
    ...
    /// Reserved byte fields must be zero according to the AEF spec.
    ...
    /// Reserved word fields must be zero according to the AEF spec.
