...
use crate::aef::{call_flag_labels, MethodToken, AefFile};
use crate::util;
...
pub(super) fn build_info_report(
    ...
    let method_tokens: Vec<MethodTokenReport> = aef
        ...
        .collect();
    WebInfoReport {
        compiler: aef.header.compiler.trim_end_matches('\0').to_string(),
        source: (!aef.header.source.is_empty()).then(|| aef.header.source.clone()),
...
pub(super) fn build_disasm_report(aef: &AefFile, output: DisassemblyOutput) -> WebDisasmReport {
    let script_hash = aef.script_hash();
    WebDisasmReport {
        script_hash_le: util::format_hash(&script_hash),
        script_hash_be: util::format_hash_be(&script_hash),
