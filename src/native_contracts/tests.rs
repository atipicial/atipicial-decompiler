...
fn native_contract_catalog_keeps_legacy_token_contracts() {
    let names: Vec<&str> = all().iter().map(|info| info.name).collect();
    assert!(
        names.contains(&"AtipicialCoin"),
        "native contract table must include AtipicialCoin for compatibility"
    );
    assert!(
        names.contains(&"AtipicialDollar"),
        "native contract table must include AtipicialDollar for compatibility"
    );
}
