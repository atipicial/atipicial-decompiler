...
/// Command line interface for the minimal Atipicial decompiler.
#[derive(Debug, Parser)]
#[command(
    author,
    version,
    about = "Inspect, disassemble, and decompile Atipicial AEF bytecode",
    long_about = "Inspect, disassemble, and decompile Atipicial AEF bytecode.\n\
        \n\
        ...
        \n\
        Companion `<AEF>.manifest.json` is auto-discovered alongside the AEF\n\
        unless `--manifest` is passed explicitly."
)]
pub struct Cli {
    /// Optional path to the companion manifest JSON file. When omitted,
    /// `<AEF>.manifest.json` next to the AEF is auto-discovered if it
    /// exists.
