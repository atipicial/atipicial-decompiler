...
use crate::decompiler::MAX_AEF_FILE_SIZE;
...
impl Cli {
    ...
    pub(super) fn resolve_manifest_path(&self, aef_path: &Path) -> Option<PathBuf> {
        if let Some(path) = &self.manifest {
            return Some(path.clone());
        }

        let mut candidate = aef_path.to_path_buf();
    ...
    /// Read a AEF file after validating its size against [`MAX_AEF_FILE_SIZE`].
    pub(super) fn read_aef_bytes(path: &Path) -> Result<Vec<u8>> {
    ...
    pub(super) fn load_manifest(&self, aef_path: &Path) -> Result<Option<ContractManifest>> {
        match self.resolve_manifest_path(aef_path) {
