...
impl AefParser {
    pub(super) fn parse_method_tokens(
        ...
        if count as usize > super::super::MAX_METHOD_TOKENS {
            return Err(AefError::TooManyMethodTokens {
        ...
        for index in 0..count as usize {
            ...
            let hash_slice = bytes
                ...
                .ok_or(AefError::UnexpectedEof { offset: hash_start })?;
            let mut hash = [0u8; 20];
            ...
            if method_len as usize > super::super::MAX_METHOD_NAME_LEN {
                return Err(AefError::InvalidMethodToken { index }.into());
            }
            ...
            let method = std::str::from_utf8(method_bytes)
                .map_err(|_| AefError::InvalidMethodToken { index })?
                .to_string();
            if method.starts_with('_') {
                return Err(AefError::MethodNameInvalid { name: method }.into());
            }
            offset = method_end;
            ...
            let has_return_value = match bytes.get(offset) {
                ...
                None => return Err(AefError::UnexpectedEof { offset }.into()),
            };
            ...
            if call_flags & !super::super::CALL_FLAGS_ALLOWED_MASK != 0 {
                return Err(AefError::CallFlagsInvalid {
                    flags: call_flags,
