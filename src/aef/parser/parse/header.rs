use super::{read_varstring, AefError, Result, MAGIC, MAX_SOURCE_LEN};

pub(super) fn read_magic(bytes: &[u8], offset: &mut usize) -> Result<[u8; 4]> {
    let magic_slice = bytes
        .get(*offset..*offset + 4)
        .ok_or(AefError::UnexpectedEof { offset: *offset })?;
    let mut magic = [0u8; 4];
    magic.copy_from_slice(magic_slice);
    *offset += 4;

    if magic != MAGIC {
        return Err(AefError::InvalidMagic {
            expected: MAGIC,
            actual: magic,
        }
        .into());
...
pub(super) fn read_compiler(bytes: &[u8], offset: &mut usize) -> Result<String> {
    ...
    *offset = compiler_end;
    ...
    let compiler = std::str::from_utf8(&compiler_bytes[..compiler_len])
        .map_err(|_| AefError::InvalidCompiler)?
        .to_string();
    Ok(compiler)
}
