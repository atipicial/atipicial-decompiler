use super::super::super::CHECKSUM_SIZE;
use super::{AefError, AefParser, Result};

fn read_u32_le(bytes: &[u8]) -> u32 {
    let mut array = [0u8; 4];
    array.copy_from_slice(bytes);
    u32::from_le_bytes(array)
}

pub(super) fn read_checksum(bytes: &[u8], checksum_start: usize) -> Result<u32> {
    let checksum_end = checksum_start + CHECKSUM_SIZE;
    let checksum_bytes =
        bytes
            .get(checksum_start..checksum_end)
            .ok_or(AefError::UnexpectedEof {
                offset: checksum_start,
            })?;
    Ok(read_u32_le(checksum_bytes))
}

pub(super) fn verify_checksum(bytes: &[u8], checksum_start: usize, checksum: u32) -> Result<()> {
    let payload = bytes
        .get(..checksum_start)
        .ok_or(AefError::UnexpectedEof { offset: 0 })?;
    let calculated = AefParser::calculate_checksum(payload);
    if checksum != calculated {
        return Err(AefError::ChecksumMismatch {
            expected: checksum,
            calculated,
        }
        .into());
    }
    Ok(())
}

pub(super) fn expect_end_of_file(bytes: &[u8], offset: usize) -> Result<()> {
    if offset != bytes.len() {
        return Err(AefError::TrailingData {
            extra: bytes.len() - offset,
        }
        .into());
    }
    Ok(())
}
