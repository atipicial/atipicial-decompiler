...
use super::super::types::{AefFile, AefHeader};
...
impl AefParser {
    /// Parse a AEF container from a byte buffer.
    ///
    /// # Errors
    /// Returns an error if the input is not a valid AEF container, if the script
    /// exceeds configured limits, or if the checksum does not match.
    pub fn parse(&self, bytes: &[u8]) -> Result<AefFile> {
        if bytes.len() as u64 > super::super::MAX_AEF_FILE_SIZE {
            return Err(AefError::FileTooLarge {
                size: bytes.len() as u64,
                max: super::super::MAX_AEF_FILE_SIZE,
            }
            .into());
        }
        if bytes.len() < FIXED_HEADER_SIZE + CHECKSUM_SIZE {
            return Err(AefError::TooShort.into());
        }
