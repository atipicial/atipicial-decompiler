...
fn rejects_source_too_long() {
    ...
    assert!(matches!(
        err,
        crate::error::Error::Aef(AefError::SourceTooLong { .. })
    ));
}
...
fn rejects_script_too_large() {
    ...
    assert!(matches!(
        err,
        crate::error::Error::Aef(AefError::ScriptTooLarge { .. })
    ));
}
...
fn rejects_files_larger_than_limit() {
    let data = vec![0u8; MAX_AEF_FILE_SIZE as usize + 1];
    let err = AefParser::new().parse(&data).unwrap_err();
    assert!(matches!(
        err,
        crate::error::Error::Aef(AefError::FileTooLarge { .. })
    ));
}
