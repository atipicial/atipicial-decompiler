...
fn rejects_overlong_method_token_name() {
    ...
    assert!(matches!(
        err,
        crate::error::Error::Aef(AefError::InvalidMethodToken { .. })
    ));
}
...
fn rejects_method_name_with_leading_underscore() {
    ...
    assert!(matches!(
        err,
        crate::error::Error::Aef(AefError::MethodNameInvalid { .. })
    ));
}
...
fn rejects_call_flags_with_unsupported_bits() {
    ...
    let err = AefParser::new().parse(&data).unwrap_err();
    match err {
        crate::error::Error::Aef(AefError::CallFlagsInvalid { flags, allowed }) => {
...
fn rejects_too_many_method_tokens() {
    ...
    assert!(matches!(
        err,
        crate::error::Error::Aef(AefError::TooManyMethodTokens { .. })
    ));
}
...
fn rejects_oversized_u64_varint_for_method_token_count() {
    ...
    assert!(matches!(
        err,
        crate::error::Error::Aef(AefError::IntegerOverflow { offset: 70 })
    ));
}
...
fn rejects_method_name_longer_than_32_bytes() {
    ...
    assert!(matches!(
        err,
        crate::error::Error::Aef(AefError::InvalidMethodToken { index: 0 })
    ));
}

#[test]
fn rejects_more_than_128_method_tokens_and_accepts_exactly_128() {
    // Reference: AefFile.Deserialize reads ReadSerializableArray<MethodToken>(128).
    ...
    let aef = AefParser::new()
        .parse(&build(128))
        .expect("128 tokens parse");
    assert_eq!(aef.method_tokens.len(), 128);

    let err = AefParser::new().parse(&build(129)).unwrap_err();
    assert!(matches!(
        err,
        crate::error::Error::Aef(AefError::TooManyMethodTokens {
            count: 129,
...
fn accepts_method_name_of_exactly_32_bytes() {
    ...
    let aef = AefParser::new().parse(&data).expect("parse succeeds");
    assert_eq!(aef.method_tokens[0].method, name);
}
