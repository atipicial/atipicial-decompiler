...
pub(crate) fn write_unpack_packstruct_helper(output: &mut String, helper_name: Option<&str>) {
    ...
    writeln!(
        output,
        "        [global::Atipicial.SmartContract.Framework.Attributes.OpCode(global::Atipicial.SmartContract.Framework.OpCode.UNPACK)]"
    )
    .unwrap();
    writeln!(
        output,
...
pub(crate) fn write_tagged_opcode_helpers(output: &mut String, helpers: &[TaggedOpcodeHelper]) {
    for helper in helpers {
        ...
        writeln!(
            output,
            "        [global::Atipicial.SmartContract.Framework.Attributes.OpCode(global::Atipicial.SmartContract.Framework.OpCode.{}, \"{tag:02X}\")]",
...
pub(crate) fn write_unresolved_call_helper(output: &mut String) {
    writeln!(
        output,
        "        private static dynamic __AtipicialDecompilerUnresolvedCall(string name, object[] args) => throw new NotImplementedException($\"Unresolved Atipicial VM call: {{name}}\");"
    )
    .unwrap();
    writeln!(output).unwrap();
}
