...
pub(super) fn write_preamble(output: &mut String) {
    ...
    writeln!(output, "using Atipicial.SmartContract.Framework;").unwrap();
    writeln!(output, "using Atipicial.SmartContract.Framework.Attributes;").unwrap();
    writeln!(output, "using Atipicial.SmartContract.Framework.Services;").unwrap();
    writeln!(output, "using Atipicial.SmartContract.Framework.Native;").unwrap();
    // The native catalog uses Atipicial's canonical contract labels while the
    // framework types retain the shorter CLR names for these contracts.
    writeln!(
        output,
        "using LedgerContract = Atipicial.SmartContract.Framework.Native.Ledger;"
    )
    .unwrap();
    writeln!(
        output,
        "using AtipicialCoin = Atipicial.SmartContract.Framework.Native.ATC;"
    )
    .unwrap();
    writeln!(
        output,
        "using AtipicialDollar = Atipicial.SmartContract.Framework.Native.GAS;"
    )
    .unwrap();
    writeln!(
        output,
        "using OracleContract = Atipicial.SmartContract.Framework.Native.Oracle;"
    )
    .unwrap();
    writeln!(
        output,
        "using PolicyContract = Atipicial.SmartContract.Framework.Native.Policy;"
    )
    .unwrap();
...
pub(super) fn write_contract_open(
    ...
    writeln!(output, "namespace AtipicialDecompiler.Generated {{").unwrap();
    ...
    if let Some(manifest) = manifest {
        ...
        if !manifest.groups.is_empty() {
            ...
            // authorise signed updates of the contract. Atipicial
    ...
    metadata::write_method_tokens_comment(output, aef);
