...
pub(super) fn render_intrinsic(
    ...
    match opcode {
        ...
        OpCode::Append => {
            ...
            RenderedExpr::new(
                format!(
                    "((Atipicial.SmartContract.Framework.List<object>){}).Add({})",
                    arg(0, expanding),
                    arg(1, expanding)
                ),
                PREC_PRIMARY,
        ...
        OpCode::Remove => {
            ...
            if receiver_type == ValueType::Map {
                ...
                RenderedExpr::new(
                    format!(
                        "((Atipicial.SmartContract.Framework.List<object>){}).RemoveAt({index})",
                        arg(0, expanding)
                    ),
                    PREC_PRIMARY,
                )
        ...
        OpCode::Clearitems => {
            ...
            if matches!(receiver_type, ValueType::Array | ValueType::Struct) {
                RenderedExpr::new(
                    format!(
                        "((Atipicial.SmartContract.Framework.List<object>){}).Clear()",
                        arg(0, expanding)
                    ),
                    PREC_PRIMARY,
                )
        ...
        OpCode::Popitem => {
            ...
            RenderedExpr::new(
                format!(
                    "((Atipicial.SmartContract.Framework.List<object>){}).PopItem()",
                    arg(0, expanding)
                ),
                PREC_PRIMARY,
            )
