...
fn nested_predicate_calls_parenthesize_ternary_operands() {
    ...
    let cases = [
        (
            OpCode::Nz,
            "__AtipicialDecompilerUnresolvedCall(\"consume\", new object[] { (BigInteger)(dynamic)(condition ? left : right) != 0 })",
        ),
        (
            OpCode::Isnull,
            "__AtipicialDecompilerUnresolvedCall(\"consume\", new object[] { (condition ? left : right) is null })",
        ),
        (
            OpCode::Istype,
            "__AtipicialDecompilerUnresolvedCall(\"consume\", new object[] { (condition ? left : right) is object })",
        ),
    ];
