...
fn csharp_multimethod_uses_structured_constant_fold() {
    ...
    let aef = fs::read(root.join("TestingArtifacts/edgecases/multi/MultiMethod.aef"))
        .expect("MultiMethod AEF");
    let manifest =
    ...
    let rendered = render_csharp_with_coverage(&aef, Some(manifest), true, false, true);
...
fn csharp_typed_declarations_preserve_loop_counter_type() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let aef = fs::read(root.join("TestingArtifacts/edgecases/LoopIf.aef")).expect("LoopIf AEF");
    let manifest = fs::read_to_string(root.join("TestingArtifacts/edgecases/LoopIf.manifest.json"))
        .expect("LoopIf manifest");
    let manifest = ContractManifest::from_json_str(&manifest).expect("manifest parsed");

    let untyped = render_csharp_with_coverage(&aef, Some(manifest.clone()), true, false, false);
    assert!(
    ...
    let typed = render_csharp_with_coverage(&aef, Some(manifest), true, false, true);
    assert!(
...
fn csharp_loopif_recovers_counting_loop_without_defeated_condition() {
    // Real LoopIf AEF: back-edge re-enters PUSH0/STLOC. Production C# path must
...
fn csharp_assert_uses_structured_body() {
    let aef = build_aef(&[0x09, 0x39, 0x11, 0x40]);
    ...
    assert!(
        rendered
            .source
            .contains("global::Atipicial.SmartContract.Framework.ExecutionEngine.Assert(false);"),
        "{}",
        rendered.source
    );
    assert!(
...
fn csharp_separates_clr_exception_transport_from_vm_payload() {
    ...
    assert!(
        rendered
            .source
            .contains("private sealed class __AtipicialDecompilerVmException : Exception"),
        "{}",
        rendered.source
    );
    assert!(
...
fn csharp_assert_preserves_non_scalar_vm_truthiness() {
    ...
    assert!(
        rendered.source.contains(
            "global::Atipicial.SmartContract.Framework.ExecutionEngine.Assert((bool)(object)(new object[0]));"
        ),
        "{}",
        rendered.source
    );
...
fn csharp_assert_message_preserves_eager_vm_message_validation() {
    ...
    // equivalent because Atipicial.Compiler.CSharp lowers it to JMPIF + ABORTMSG.
    let aef = build_aef(&[0x11, 0x0C, 0x01, 0xFF, 0xE1, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"{
    ...
    assert!(
        rendered.source.contains(
            "[global::Atipicial.SmartContract.Framework.Attributes.OpCode(global::Atipicial.SmartContract.Framework.OpCode.ASSERTMSG)]"
        ),
    ...
    assert!(
        rendered.source.contains(
            "private static extern void __AtipicialDecompilerAssertMessage(bool condition, string message);"
        ),
        "{}",
        rendered.source
    );
    assert!(
        rendered
            .source
            .contains("            __AtipicialDecompilerAssertMessage(1 != 0,"),
        "{}",
        rendered.source
    );
    assert!(
...
fn csharp_assert_message_helper_call_ignores_parameter_shadowing() {
    ...
    let aef = build_aef(&[0x57, 0x00, 0x02, 0x11, 0x79, 0xE1, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"{
            ...
            "abi": { "methods": [{
                ...
                "parameters": [
                    {
                        "name": "__AtipicialDecompilerAssertMessage",
                        "type": "Any"
                    },
                    {
                        "name": "HelperNameCollision",
    ...
    assert!(
        rendered.source.contains(
            "private static extern void __AtipicialDecompilerAssertMessage_1(bool condition, string message);"
        ) && rendered.source.contains(
            "__AtipicialDecompilerAssertMessage_1(1 != 0, (string)(object)(HelperNameCollision));"
        ),
...
fn csharp_assert_uses_globally_qualified_framework_intrinsic() {
    let aef = build_aef(&[0x11, 0x39, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"{
    ...
    assert!(
        rendered
            .source
            .contains("global::Atipicial.SmartContract.Framework.ExecutionEngine.Assert(1 != 0);"),
...
fn csharp_assert_message_uses_globally_qualified_opcode_attribute() {
    let aef = build_aef(&[0x11, 0x12, 0xE1, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"{
            "name": "OpCode",
            "abi": { "methods": [{
    ...
    assert!(
        rendered
            .source
            .contains("__AtipicialDecompilerAssertMessage(1 != 0, (string)(object)(2));"),
        "{}",
        rendered.source
    );
}
...
fn csharp_failure_statements_preserve_semantics_and_abort_warning() {
    ...
    let render = |name: &str, script: &[u8]| {
        render_csharp_with_coverage(&build_aef(script), Some(manifest(name)), true, false, true)
    };
    ...
    assert!(
        throw
            .source
            .contains("throw new __AtipicialDecompilerVmException(1);"),
        "{}",
        throw.source
    );
    assert!(
...
fn csharp_trace_failure_statements_preserve_semantics_and_abort_warning() {
    ...
    let render = |name: &str, script: &[u8]| {
        render_csharp_with_coverage(&build_aef(script), Some(manifest(name)), true, true, true)
    };
...
fn csharp_trace_assert_converts_numeric_local_to_boolean() {
    ...
    let aef = build_aef(&[0x57, 0x01, 0x00, 0x11, 0x70, 0x68, 0x39, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"{
    ...
    let rendered = render_csharp_with_coverage(&aef, Some(manifest), true, true, true);
...
fn csharp_trace_assert_converts_named_numeric_parameter_to_boolean() {
    ...
    let aef = build_aef(&[0x57, 0x00, 0x01, 0x78, 0x39, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"{
    ...
    for typed_declarations in [false, true] {
        ...
        assert!(
            rendered.source.contains(
                "global::Atipicial.SmartContract.Framework.ExecutionEngine.Assert(amount != 0);"
            ),
            "{}",
            rendered.source
        );
...
fn csharp_trace_assert_prefers_emitted_parameter_name_over_raw_slot_syntax() {
    ...
    let aef = build_aef(&[0x57, 0x00, 0x02, 0x78, 0x39, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"{
    ...
    assert!(
        rendered
            .source
            .contains("global::Atipicial.SmartContract.Framework.ExecutionEngine.Assert(arg1);"),
