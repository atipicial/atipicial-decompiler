...
use crate::{ContractManifest, AefParser};
...
fn build_aef_with_single_token(
    script: &[u8],
    hash: [u8; 20],
    method: &str,
...
fn sample_aef() -> Vec<u8> {
    // Build a minimal AEF with script: PUSH0, PUSH1, ADD, RET
    build_aef(&[0x10, 0x11, 0x9E, 0x40])
}
...
fn load_testing_aef(name: &str) -> Vec<u8> {
    fs::read(testing_artifact_dir().join(name)).expect("failed to read AEF artifact")
}

fn try_load_testing_aef(name: &str) -> Option<Vec<u8>> {
...
fn debug_inferred_method_starts_contract_delegate() {
    let aef_bytes = load_testing_aef("Contract_Delegate.aef");
    let manifest = load_testing_manifest("Contract_Delegate.manifest.json");
    let decompilation = Decompiler::new()
        .decompile_bytes_with_manifest(&aef_bytes, Some(manifest.clone()), OutputFormat::All)
