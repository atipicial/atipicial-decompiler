...
fn csharp_unknown_source_and_unresolved_call_use_whole_method_fallback() {
    let unknown_source =
        render_csharp_with_coverage(&build_aef(&[0x68, 0x40]), None, true, false, true);
    ...
    let unresolved_call =
        render_csharp_with_coverage(&build_aef(&[0x34, 0x7F, 0x40]), None, true, false, true);
    ...
    assert!(unresolved_call
        .source
        .contains("__AtipicialDecompilerUnresolvedCall"));
}
...
fn csharp_typed_map_temporary_keeps_its_receiver_type() {
    ...
    let rendered =
        render_csharp_with_coverage(&build_aef(&script), Some(manifest), false, false, true);
...
fn csharp_coverage_retains_same_name_overloads_at_one_offset() {
    ...
    let rendered = render_csharp_with_coverage(
        &build_aef(&[crate::instruction::OpCode::Ret.byte()]),
        Some(manifest),
        true,
        false,
        true,
...
fn csharp_synthetic_script_entry_exposes_initslot_args_and_preserves_return() {
    ...
    // it).
    let aef_bytes = build_aef(&[0x57, 0x00, 0x01, 0x78, 0xD8, 0xAA, 0x40]);
...
fn csharp_keeps_explicit_return_value_in_non_void_methods() {
    ...
    let aef_bytes = build_aef(&[0x11, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"
            {
                "name": "ReturnsInt",
