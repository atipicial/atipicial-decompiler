...
fn collect_aef_files(path: &Path, files: &mut Vec<PathBuf>) -> io::Result<()> {
...
fn csharp_corpus_has_zero_structured_fallback() {
    let root = env::var_os("ATC_CSHARP_CORPUS_DIR").map_or_else(
        || PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("TestingArtifacts"),
        PathBuf::from,
    );
    let mut files = Vec::new();
    collect_aef_files(&root, &mut files)
        .unwrap_or_else(|error| panic!("failed to collect {}: {error}", root.display()));
    files.sort();
    ...
    for aef_path in files {
        ...
        let id = artifact_id(&aef_path, &root);
        let bytes = fs::read(&aef_path)
            .unwrap_or_else(|error| panic!("failed to read {}: {error}", aef_path.display()));
        ...
        let decompilation = decompilation
            .unwrap_or_else(|error| panic!("failed to decompile {}: {error}", aef_path.display()));
    ...
    assert!(decompiled > 0, "no repository AEF corpus artifacts found");
...
fn assert_pinned_incomplete_baseline(root: &Path, incomplete_locations: &BTreeSet<String>) {
    ...
    // them, so never re-baseline from an unpinned `../atipicial-devpack-dotnet`.
