...
fn legacy_statement_to_csharp_converts_known_forms() {
    ...
    assert_eq!(
        ...
        "global::Atipicial.SmartContract.Framework.ExecutionEngine.Assert((bool)(object)((loc0 is null)));"
    );
    // Assert messages use explicit conversion regardless of source type.
    assert_eq!(
        legacy_statement_to_csharp("assert(min(a, b) > 0, \"e\" cat code);"),
        "__AtipicialDecompilerAssertMessage((bool)(object)(BigInteger.Min(a, b) > 0), (string)(object)(\"e\" + code));"
    );
    assert_eq!(
        legacy_statement_to_csharp("assert(x > 0, code);"),
        "__AtipicialDecompilerAssertMessage((bool)(object)(x > 0), (string)(object)(code));"
    );
    ...
    // `throw(value);` (high-level pseudocode for ATC's THROW opcode)
    // becomes `throw new Exception(value);` in C# — ATC accepts any
    ...
    assert_eq!(
        ...
        "global::Atipicial.SmartContract.Framework.ExecutionEngine.Assert((bool)(object)(x > 0));"
    );
    assert_eq!(
        legacy_statement_to_csharp("assert(x > 0, \"must be positive\");"),
        "__AtipicialDecompilerAssertMessage((bool)(object)(x > 0), (string)(object)(\"must be positive\"));"
    );
    assert_eq!(
        legacy_statement_to_csharp("assert(1, 2);"),
        "__AtipicialDecompilerAssertMessage(1 != 0, (string)(object)(2));"
    );
    // Don't be fooled by commas inside the condition expression.
    assert_eq!(
        legacy_statement_to_csharp("assert(foo(a, b));"),
        "global::Atipicial.SmartContract.Framework.ExecutionEngine.Assert((bool)(object)(foo(a, b)));"
    );
    assert_eq!(
        legacy_statement_to_csharp("assert(null);"),
        "global::Atipicial.SmartContract.Framework.ExecutionEngine.Assert(false);"
    );
    // ATC arithmetic helpers — the high-level lift emits `abs/min/max/pow`
    ...
    // Extended ATC arithmetic / buffer helpers — `BigInteger.X` for
    // ones .NET provides directly, `Helper.X` (Atipicial SmartContract
    ...
    // `keys(m)`, `values(m)`, `reverse_items(arr)` are ATC-flavoured
    // identifiers that don't compile. Rewrite to standard
    // .NET / Atipicial Map accessors.
    assert_eq!(
