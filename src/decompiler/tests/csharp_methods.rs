...
fn csharp_resolves_internal_calls_to_method_names() {
    ...
    assert!(
        !csharp.contains("__AtipicialDecompilerUnresolvedCall"),
...
fn csharp_ambiguous_internal_call_emits_unresolved_call_helper() {
    ...
    let csharp = Decompiler::new()
        .with_trace_comments(false)
        .decompile_bytes_with_manifest(&build_aef(&script), Some(manifest), OutputFormat::All)
        .expect("decompile succeeds")
        .csharp
        .expect("csharp output");

    assert!(
        csharp.contains("__AtipicialDecompilerUnresolvedCall(\"call_0x0014\""),
        "ambiguous call should remain explicit in the C# body: {csharp}"
    );
    assert!(
        csharp.contains("private static dynamic __AtipicialDecompilerUnresolvedCall"),
...
fn csharp_manifest_void_internal_call_is_a_statement() {
    ...
    // 0x0002: RET
    // 0x0003: RET
    let aef_bytes = build_aef(&[0x34, 0x03, 0x40, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"
            {
                "name": "VoidCall",
...
fn csharp_offsetless_manifest_void_internal_call_is_a_statement() {
    ...
    let aef_bytes = build_aef(&[0x34, 0x00, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"
            {
...
fn csharp_offsetless_manifest_void_resolved_calla_is_a_statement() {
    ...
    let aef_bytes = build_aef(&[0x0A, 0x00, 0x00, 0x00, 0x00, 0x36, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"
            {
...
fn csharp_manifest_void_tail_call_does_not_return_ambient_stack_value() {
    ...
    let aef_bytes = build_aef(&[0x11, 0x22, 0x02, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"
            {
                "name": "VoidTailCall",
...
fn csharp_manifest_void_internal_call_underflow_keeps_call_visible() {
    ...
    let aef_bytes = build_aef(&[0x57, 0x00, 0x01, 0x40, 0x34, 0xFC, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"
            {
...
fn csharp_manifest_value_tail_call_underflow_still_returns_call() {
    ...
    let aef_bytes = build_aef(&[0x57, 0x00, 0x01, 0x40, 0x22, 0xFC]);
    let manifest = ContractManifest::from_json_str(
        r#"
            {
...
fn csharp_manifest_value_internal_call_still_produces_a_value() {
    ...
    // 0x0002: RET
    // 0x0003: PUSH1; RET
    let aef_bytes = build_aef(&[0x34, 0x03, 0x40, 0x11, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"
            {
                "name": "ValueCall",
...
fn csharp_uses_label_style_for_transfer_placeholders() {
    ...
    let aef_bytes = build_aef(&[0x3D, 0x06, 0x11, 0x12, 0x9E, 0x75, 0x40]);
...
fn csharp_mismatch_offset_emits_script_entry_and_manifest_method() {
    ...
    let aef_bytes = build_aef(&[0x11, 0x40, 0x12, 0x40]);
    let manifest = ContractManifest::from_json_str(
        r#"
            {
                "name": "OffsetMismatch",
