...
#[test]
fn header_surfaces_aef_compiler_and_source_fields() {
    // The AEF header carries `compiler` (always set in practice) and
    ...
    // skipped (the test harness's `build_aef` writes `compiler =
    // "test"` and an empty source, so we exercise the present /
    // absent paths together).
    let aef_bytes = sample_aef();
    let decompilation = Decompiler::new()
        .decompile_bytes_with_manifest(&aef_bytes, None, OutputFormat::All)
        .expect("decompile succeeds");
    ...
    assert!(
        ...
        "high-level should surface the AEF compiler field:\n{high_level}"
    );
    assert!(
    ...
    assert!(
        ...
        "C# header should surface the AEF compiler field at the C# indent:\n{csharp}"
    );
    assert!(
...
fn csharp_header_surfaces_inferred_patterns_and_language() {
    let mut aef_bytes = sample_aef();
    let compiler = b"Atipicial.Compiler.CSharp 3";
    aef_bytes[4..4 + compiler.len()].copy_from_slice(compiler);
    let checksum_offset = aef_bytes.len() - std::mem::size_of::<u32>();
    let checksum = AefParser::calculate_checksum(&aef_bytes[..checksum_offset]);
    aef_bytes[checksum_offset..].copy_from_slice(&checksum.to_le_bytes());
    let manifest = ContractManifest::from_json_str(
        r#"{
            "name": "Token",
    ...
    assert!(csharp.contains("        // inferred standards: AEP-17"));
    assert!(csharp.contains("        // inferred patterns: AEP-17"));
    assert!(csharp.contains("        // inferred language: C#"));
    assert!(csharp.contains("        // pattern confidence: high"));
    assert!(csharp.contains("using OracleContract = Atipicial.SmartContract.Framework.Native.Oracle;"));
    assert!(csharp.contains("using PolicyContract = Atipicial.SmartContract.Framework.Native.Policy;"));
}

#[test]
fn csharp_header_renders_method_tokens_block() {
    // The high-level renderer surfaces `// method tokens declared in
    // AEF` so a reader can cross-reference each CALLT call against
    // its native contract / call flags. The C# header silently
    // dropped the table, leaving readers to scrape the AEF
    // separately. Render it as a comment block (parity with the
    // existing `// permissions:` / `// groups:` blocks) — Atipicial
    ...
    let aef_bytes = build_aef_with_single_token(&[0x40], stdlib_hash, "Serialize", 1, true, 0x0F);
    ...
    let csharp = decompilation.csharp.as_deref().expect("csharp output");
    assert!(
        csharp.contains("// method tokens declared in AEF:"),
...
fn csharp_header_omits_method_tokens_block_when_none() {
    ...
    // `// method tokens declared in AEF:` orphan).
    let aef_bytes = sample_aef();
    let decompilation = Decompiler::new()
        .decompile_bytes_with_manifest(&aef_bytes, None, OutputFormat::All)
        .expect("decompile succeeds");
    ...
    assert!(
        !csharp.contains("method tokens declared in AEF"),
        "no token block expected when AEF has no method tokens:\n{csharp}"
    );
}
