//! Return types for native method tokens with stable Atipicial C# framework APIs.
...
pub(crate) fn lookup(
    ...
    match (hint.contract, method) {
        ...
        ("AtipicialDollar" | "AtipicialCoin", "Symbol") => Some(return_type(ValueType::ByteString, "string")),
        ("AtipicialDollar" | "AtipicialCoin", "Decimals") => Some(return_type(ValueType::Integer, "byte")),
        (
            "AtipicialDollar" | "AtipicialCoin",
            "BalanceOf" | "GetGasPerBlock" | "TotalSupply" | "UnclaimedGas",
        ) => Some(return_type(ValueType::Integer, "BigInteger")),
        ("AtipicialDollar" | "AtipicialCoin", "Transfer") => Some(return_type(ValueType::Boolean, "bool")),
        ("AtipicialCoin", "GetRegisterPrice") => Some(return_type(ValueType::Integer, "long")),
        ("AtipicialCoin", "RegisterCandidate" | "UnregisterCandidate" | "Vote") => {
            Some(return_type(ValueType::Boolean, "bool"))
        }
        ...
        ("AtipicialCoin", "GetCandidates") => {
            Some(return_type(ValueType::Array, "(ECPoint, BigInteger)[]"))
        }
        ("AtipicialCoin", "GetAllCandidates") => Some(return_type(
            ValueType::InteropInterface,
            "Iterator<(ECPoint, BigInteger)>",
        )),
        ("AtipicialCoin", "GetCandidateVote") => Some(return_type(ValueType::Integer, "BigInteger")),
        ("AtipicialCoin", "GetCommittee" | "GetNextBlockValidators") => {
            Some(return_type(ValueType::Array, "ECPoint[]"))
        }
        ("AtipicialCoin", "GetCommitteeAddress") => Some(return_type(ValueType::ByteString, "UInt160")),
        ("AtipicialCoin", "GetAccountState") => {
            Some(return_type(ValueType::InteropInterface, "AtipicialAccountState"))
        }
...
mod tests {
    ...
    const ATC: &str = "F563EA40BC283D4D0E05C48EA305B3F2A07340EF";
    ...
    fn maps_framework_native_contract_returns() {
        ...
        let balance = lookup(Some(ATC), "balanceOf", Some(0x0F)).unwrap();
    ...
    fn maps_additional_framework_native_returns() {
        ...
        let candidates = lookup(Some(ATC), "getCandidates", Some(0x0F)).unwrap();
        assert_eq!(candidates.value_type, ValueType::Array);
        assert_eq!(candidates.csharp_type, "(ECPoint, BigInteger)[]");

        let candidate_iterator = lookup(Some(ATC), "getAllCandidates", Some(0x0F)).unwrap();
        ...
        let account_state = lookup(Some(ATC), "getAccountState", Some(0x0F)).unwrap();
        assert_eq!(account_state.value_type, ValueType::InteropInterface);
        assert_eq!(account_state.csharp_type, "AtipicialAccountState");
