...
use super::*;
use crate::aef::AefHeader;

fn aef(compiler: &str, source: &str) -> AefFile {
    AefFile {
        header: AefHeader {
            magic: *b"AEF3",
...
fn manifest_standard_is_high_confidence() {
    let manifest: ContractManifest = serde_json::from_str(
        r#"{"name":"Token","abi":{"methods":[],"events":[]},"supportedstandards":["AEP-17"]}"#,
    )
    .expect("manifest fixture");
    let info = identify_patterns(&aef("Atipicial.Compiler.CSharp 3", ""), &[], Some(&manifest));
    assert_eq!(info.standards, vec!["AEP-17"]);
    assert_eq!(info.language.as_deref(), Some("C#"));
    assert_eq!(info.confidence, PatternConfidence::High);
    assert!(info
        .evidence
        .iter()
        .any(|entry| entry.source == "aef.header.compiler"));
}

#[test]
fn weak_metadata_does_not_claim_a_standard() {
    let info = identify_patterns(&aef("", "contract.py"), &[], None);
...
fn unsupported_source_metadata_is_not_claimed_as_a_renderer() {
    for (compiler, source) in [
        ...
        let info = identify_patterns(&aef(compiler, source), &[], None);
...
fn short_csharp_compiler_tags_infer_language() {
    for compiler in ["cs", "cs__", "cs 3.7", "CSharp", "Atipicial.Compiler.CSharp 3"] {
        let info = identify_patterns(&aef(compiler, ""), &[], None);
        assert_eq!(
...
fn compiler_tags_require_explicit_csharp_tokens() {
    for compiler in ["notcsharp", "CSharpX", "Atipicial.Compiler.CSharpX", "cscompiler"] {
        let info = identify_patterns(&aef(compiler, ""), &[], None);
...
fn events_manifest_reports_events_pattern_with_evidence() {
    ...
    .expect("manifest fixture");
    let info = identify_patterns(&aef("evnt", ""), &[], Some(&manifest));
...
fn storage_runtime_and_account_syscalls_report_behavior_patterns() {
    ...
    let info = identify_patterns(&aef("", ""), &instructions, None);
...
fn wildcard_permissions_are_reported_as_behavior_evidence() {
    let manifest: ContractManifest = serde_json::from_str(
        ...
        .expect("manifest fixture");
    let info = identify_patterns(&aef("", ""), &[], Some(&manifest));
    assert_eq!(
        info.patterns,
...
#[test]
fn royalty_info_reports_aep24_and_royalties_patterns() {
    let manifest: ContractManifest = serde_json::from_str(
            r#"{"name":"Royalty","abi":{"methods":[{"name":"royaltyInfo","parameters":[],"returntype":"Array"}],"events":[]}}"#,
        )
        .expect("manifest fixture");
    let info = identify_patterns(&aef("", ""), &[], Some(&manifest));
    assert_eq!(info.standards, vec!["AEP-24"]);
    assert_eq!(info.patterns, vec!["AEP-24", "royalties"]);
    assert!(info
        .evidence
        .iter()
...
fn token_payment_callbacks_report_receiver_behavior_without_standard_guess() {
    let manifest: ContractManifest = serde_json::from_str(
            r#"{"name":"Receiver","abi":{"methods":[{"name":"onAEP17Payment","parameters":[],"returntype":"Void"}],"events":[]}}"#,
        )
        .expect("manifest fixture");
    let info = identify_patterns(&aef("", ""), &[], Some(&manifest));
...
fn method_tokens_and_calls_are_reported_without_standard_guesses() {
    let aef = AefFile {
        method_tokens: vec![crate::aef::MethodToken {
            hash: [0; 20],
...
fn native_oracle_method_tokens_report_oracle_behavior() {
    ...
    let info = identify_patterns(&aef, &[], None);
    assert_eq!(
        info.patterns,
