...
pub struct PatternInfo {
    /// Declared or inferred Atipicial standards, sorted and deduplicated.
    ...
    /// Raw compiler identifier from the AEF header.
...
pub fn identify_patterns(
    ...
    if !aef.method_tokens.is_empty() {
        ...
        evidence.push(PatternEvidence {
            source: "aef.method_tokens".to_string(),
            value: aef.method_tokens.len().to_string(),
        });
    }
    for token in &aef.method_tokens {
            ...
            .filter(|hint| hint.has_exact_method())
        {
            ...
            evidence.push(PatternEvidence {
                source: "aef.method_tokens.native".to_string(),
                value: label.clone(),
            });
    ...
    let compiler = (!aef.header.compiler.trim().is_empty()).then(|| aef.header.compiler.clone());
    if let Some(compiler) = compiler.as_deref() {
        evidence.push(PatternEvidence {
            source: "aef.header.compiler".to_string(),
            value: compiler.to_string(),
        });
    }
    if !aef.header.source.trim().is_empty() {
        evidence.push(PatternEvidence {
            source: "aef.header.source".to_string(),
            value: aef.header.source.clone(),
        });
    }
    let compiler_language = compiler.as_deref().and_then(language::infer_language);
    let language =
        compiler_language.or_else(|| language::infer_language_from_source(&aef.header.source));

    let confidence = if strong_standard {
        PatternConfidence::High
    } else if compiler_language.is_some()
        || patterns.contains("AEP-17")
        || patterns.contains("AEP-11")
