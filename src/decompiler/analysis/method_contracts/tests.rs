...
use crate::aef::{MethodToken, AefFile, AefHeader};
...
fn analyze_with_tokens(
    ...
    let call_graph = build_call_graph(&aef, &instructions, manifest);
...
fn sorts_and_deduplicates_call_graph_methods_by_offset() {
    ...
    let mut call_graph = build_call_graph(&aef, &instructions, Some(&manifest));
