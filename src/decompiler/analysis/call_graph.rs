//! Call graph construction for Atipicial scripts.
...
pub enum CallTarget {
    ...
    /// Call to an entry in the AEF method-token table (CALLT).
    MethodToken {
        /// Index into the AEF `method_tokens` table.
        index: u16,
    ...
    for (index, instr) in instructions.iter().enumerate() {
        match instr.opcode {
            ...
            OpCode::CallT => {
                ...
                let token = aef.method_tokens.get(index as usize);
