...
// AEF scripts are bounded (~1 MB), so these conversions are structurally safe on
...
impl<'a> CfgBuilder<'a> {
    /// Resolve a jump target offset.  Atipicial VM jump offsets are relative to the
    ...
    pub(super) fn try_targets(
        ...
        // Atipicial VM: try handler offsets are relative to the **opcode position**
