...
// AEF scripts are bounded (~1 MB), so these conversions are structurally safe.
#![allow(
...
impl HighLevelEmitter {
    /// Resolve a forward jump target.  Atipicial VM jump offsets are relative to the
    /// **opcode position** (start of the instruction), NOT the end.
    /// Reference: atipicial-vm `ExecuteJumpOffset` → `IP + offset`.
    ...
    /// Resolve a jump target (forward or backward).  Atipicial VM jump offsets are
