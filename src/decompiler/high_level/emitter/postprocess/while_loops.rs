...
impl HighLevelEmitter {
    ...
    pub(crate) fn rewrite_if_goto_to_while(statements: &mut Vec<String>) {
        ...
        // in-cap AEF that emits many `label_X:` lines with no following `if`
    ...
    /// produced by the Atipicial C# compiler; lifting it removes both the label
    ...
    pub(crate) fn rewrite_label_goto_to_loop(statements: &mut [String]) {
        ...
        // vector. Without this the pass is O(labels × N): a crafted in-cap AEF
