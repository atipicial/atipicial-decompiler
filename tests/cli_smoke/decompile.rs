...
fn decompile_command_outputs_csharp_by_default() {
    ...
    let assert = atipicial_decompiler_cmd()
        .arg("decompile")
        .arg(&aef_path)
        .assert()
        .success()
        .stdout(contains("namespace AtipicialDecompiler.Generated"))
        .stdout(contains("public class AtipicialContract : SmartContract"));
...
fn decompile_command_uses_typed_csharp_declarations_by_default() {
    let aef_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("TestingArtifacts/edgecases/LoopIf.aef");
...
fn decompile_command_accepts_inline_single_use_temps_flag() {
    ...
    atipicial_decompiler_cmd()
        ...
        .assert()
        .success()
        .stdout(contains("contract AtipicialContract"));
}
...
fn decompile_command_supports_json_format() {
    ...
    assert!(value["csharp"]
        ...
        .contains("namespace AtipicialDecompiler.Generated"));
    assert!(value["csharp"]
        .as_str()
        .expect("csharp string")
