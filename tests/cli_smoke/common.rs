...
pub(crate) fn atipicial_decompiler_cmd() -> Command {
    cargo_bin_cmd!("atipicial-decompiler")
}
...
pub(crate) fn build_sample_aef() -> Vec<u8> {
...
pub(crate) fn build_aef_with_no_tokens() -> Vec<u8> {
    let script = [0x40];
    let mut data = Vec::new();
    data.extend_from_slice(b"AEF3");
    let mut compiler = [0u8; 64];
...
pub(crate) fn build_aef_with_unknown_opcode() -> Vec<u8> {
...
pub(crate) fn write_oversize_aef(path: &Path) {
    let file = File::create(path).expect("create oversize aef");
    file.set_len(atipicial_decompiler::aef::MAX_AEF_FILE_SIZE + 1)
        .expect("set oversize aef length");
}
