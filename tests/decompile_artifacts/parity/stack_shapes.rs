...
fn foreach_contract_methods_use_structured_loops_without_unlifted_cfg_warnings() {
    ...
    let aef_path = artifacts_dir.join("Contract_Foreach.aef");
    ...
    let aef_bytes = fs::read(&aef_path)
        .unwrap_or_else(|err| panic!("failed to read {}: {err}", aef_path.display()));
...
fn trycatch_handlers_do_not_underflow_on_catch_exception_slot_store() {
    ...
    let aef_path = artifacts_dir.join("Contract_TryCatch.aef");
...
#[test]
fn aep11_balance_of_istype_and_unpack_stack_modeling_avoids_underflow_warnings() {
    let artifacts_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("TestingArtifacts/devpack");
    if !artifacts_dir.is_dir() {
        eprintln!(
            "Skipping devpack parity test: {} not found",
    ...
    let aef_path = artifacts_dir.join("Contract_AEP11.aef");
    let manifest_path = artifacts_dir.join("Contract_AEP11.manifest.json");
    if !aef_path.is_file() || !manifest_path.is_file() {
        eprintln!(
            "Skipping devpack parity test: missing {} or {}",
            aef_path.display(),
...
fn reentrancy_unknown_unpack_preserves_stack_for_reverse3_swap_helpers() {
    ...
    let aef_path = artifacts_dir.join("Contract_Reentrancy.aef");
...
fn tuple_unknown_unpack_preserves_stack_for_drop_stloc_drop_sequence() {
    ...
    let aef_path = artifacts_dir.join("Contract_Tuple.aef");
