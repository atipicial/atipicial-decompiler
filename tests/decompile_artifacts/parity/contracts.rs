...
fn edgecase_csharp_output_stays_high_level() {
    ...
    let artifacts = [
        ("LoopIf.aef", "LoopIf.manifest.json"),
        ("events/Events.aef", "events/Events.manifest.json"),
        ("multi/MultiMethod.aef", "multi/MultiMethod.manifest.json"),
        (
            "permissions/Permissions.aef",
            "permissions/Permissions.manifest.json",
        ),
    ];

    for (aef_name, manifest_name) in artifacts {
        let aef_path = root.join(aef_name);
        let manifest_path = root.join(manifest_name);
        if !aef_path.is_file() || !manifest_path.is_file() {
            eprintln!("Skipping missing edgecase artifact {}", aef_path.display());
            continue;
        }
        let aef = fs::read(&aef_path).expect("read edgecase AEF");
        ...
        let result = Decompiler::new()
            ...
            .decompile_bytes_with_manifest(&aef, Some(manifest), OutputFormat::CSharp)
...
/// Every supported in-repo AEF + companion manifest must decompile to a
/// human-readable  `SmartContract` C# skeleton: envelope, ABI methods with
...
fn all_supported_artifacts_decompile_to_high_level_csharp_contracts() {
    ...
    let mut aef_paths = Vec::new();
    fn collect_aefs(dir: &Path, out: &mut Vec<std::path::PathBuf>) {
        ...
        for entry in entries.flatten() {
            let path = entry.path();
            if path.is_dir() {
                // Skip generated decompiled tree except embedded sample AEF parent.
                if path.file_name().and_then(|n| n.to_str()) == Some("decompiled") {
                    let embedded = path.join("embedded");
                    if embedded.is_dir() {
                        collect_aefs(&embedded, out);
                    }
                    continue;
                }
                collect_aefs(&path, out);
            } else if path.extension().and_then(|e| e.to_str()) == Some("aef") {
                out.push(path);
            }
        }
    }
    collect_aefs(&root, &mut aef_paths);
    aef_paths.sort();
    assert!(
        !aef_paths.is_empty(),
        "expected at least one AEF under TestingArtifacts"
    );

    let mut checked = 0usize;
    for aef_path in &aef_paths {
        let rel = aef_path
            .strip_prefix(&root)
            .unwrap_or(aef_path.as_path())
            .to_string_lossy()
            .replace('\\', "/");
        if skip_substrings
            .iter()
        ...
        let stem = aef_path.file_stem().and_then(|s| s.to_str()).unwrap_or("");
        let manifest_path = aef_path.with_file_name(format!("{stem}.manifest.json"));
        assert!(
        ...
        let aef = fs::read(aef_path).unwrap_or_else(|e| panic!("read {}: {e}", aef_path.display()));
        let manifest = ContractManifest::from_json_str(
        ...
        let result = Decompiler::new()
            ...
            .unwrap_or_else(|e| panic!("decompile {}: {e}", aef_path.display()));

        let csharp = result
            .csharp
            .as_deref()
            .unwrap_or_else(|| panic!("missing C# for {}", aef_path.display()));
        let high_level = result
            .high_level
            .as_deref()
            .unwrap_or_else(|| panic!("missing high-level for {}", aef_path.display()));
