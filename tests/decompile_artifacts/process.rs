...
use atipicial_decompiler::{Decompiler, OutputFormat};
use std::fs;
use std::path::Path;
...
use crate::csharp_embed::{
    extract_section, unescape_verbatim, MANIFEST_PREFIX, MANIFEST_SUFFIX, AEF_PREFIX, AEF_SUFFIX,
};
...
pub(crate) fn process_artifact(
    ...
    match &artifact.kind {
        ...
        Artifact::AefManifest {
            aef_path,
            manifest_path,
        } => process_aef_contract(
            decompiler,
            aef_path,
            manifest_path,
            &artifact.output_base,
...
fn process_csharp_contract(
    ...
    let aef_base64 = extract_section(&source, AEF_PREFIX, AEF_SUFFIX)
        .unwrap_or_else(|| panic!("AEF section not found in {}", source_path.display()));
    let aef_base64 = unescape_verbatim(aef_base64)
        .chars()
        .filter(|c| !c.is_whitespace())
        .collect::<String>();
    let aef_bytes = BASE64
        .decode(aef_base64)
        .unwrap_or_else(|err| panic!("invalid base64 in {}: {err}", source_path.display()));

    let aef_path = output_base.with_extension("aef");
    let manifest_path = output_base.with_extension("manifest.json");

    create_parent(&aef_path);
    create_parent(&manifest_path);

    fs::write(&aef_path, &aef_bytes)
        .unwrap_or_else(|err| panic!("failed to write {}: {err}", aef_path.display()));
...
fn process_aef_contract(
    ...
    assert!(aef_path.is_file(), "AEF missing: {}", aef_path.display());
    assert!(
...
fn decompile_and_write_outputs(
    ...
    match decompiler.decompile_file_with_manifest(aef_path, Some(manifest_path), OutputFormat::All)
    {
        Ok(result) => {
            assert!(
        ...
        Err(err) => {
            if find_entry(id, expected_invalid).is_some() {
                ...
                panic!("failed to decompile {}: {err}", aef_path.display());
            }
        }
    }
}
