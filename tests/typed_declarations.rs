...
use atipicial_decompiler::{ContractManifest, Decompiler, AefParser, OutputFormat};
...
/// Read a `(aef, manifest)` pair from TestingArtifacts.
...
fn typed_declarations_annotate_inferred_integer_locals() {
    ...
    let (aef, manifest) = artifact("edgecases/LoopIf");

    // Default (off): body locals render as `var loc0`, never typed.
    let untyped = decompile_csharp(&aef, manifest.as_deref(), false);
    assert!(
    ...
    let typed = decompile_csharp(&aef, manifest.as_deref(), true);
    assert!(
...
fn typed_declarations_off_matches_default() {
    ...
    let default = Decompiler::new()
        .decompile_bytes_with_manifest(&aef, parsed, OutputFormat::CSharp)
        .unwrap()
        .csharp
        .unwrap_or_default();
    let off = decompile_csharp(&aef, manifest.as_deref(), false);
    assert_eq!(
        default, off,
...
fn typed_declarations_emit_static_slots_once_at_class_scope() {
    ...
    let csharp = decompile_csharp(&aef, None, true);
