...
//!
//! These build real AEF containers (or reuse a real artifact) and run the full
...
use atipicial_decompiler::{Decompiler, AefParser, OutputFormat, ReturnBehavior};

fn repo_root() -> std::path::PathBuf {
    std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
}

/// Minimal AEF3 wrapper around a script (mirrors the in-crate test helper).
fn build_aef(script: &[u8]) -> Vec<u8> {
    let mut data = Vec::new();
    data.extend_from_slice(b"AEF3");
...
fn structured_ir_decodes_signed_wide_integer() {
    let mut script = vec![atipicial_decompiler::OpCode::Pushint128.byte()];
    script.extend([0xFF; 16]);
    script.push(atipicial_decompiler::OpCode::Ret.byte());
    let aef = build_aef(&script);
...
fn structured_ir_recovers_pack_families_and_constant_unpack() {
    let cases = [
        (
            vec![
                atipicial_decompiler::OpCode::Push2.byte(),
                atipicial_decompiler::OpCode::Push1.byte(),
                atipicial_decompiler::OpCode::Push2.byte(),
                atipicial_decompiler::OpCode::Pack.byte(),
                atipicial_decompiler::OpCode::Ret.byte(),
            ],
            "[1, 2]",
        ),
        (
            vec![
                ...
                atipicial_decompiler::OpCode::Push1.byte(),
                atipicial_decompiler::OpCode::Push2.byte(),
                atipicial_decompiler::OpCode::Packstruct.byte(),
                atipicial_decompiler::OpCode::Ret.byte(),
            ],
            "struct[1, 2]",
        ),
        (
            vec![
                atipicial_decompiler::OpCode::Push4.byte(),
                atipicial_decompiler::OpCode::Push3.byte(),
                atipicial_decompiler::OpCode::Push2.byte(),
                atipicial_decompiler::OpCode::Push1.byte(),
                atipicial_decompiler::OpCode::Push2.byte(),
                atipicial_decompiler::OpCode::Packmap.byte(),
                atipicial_decompiler::OpCode::Ret.byte(),
            ],
            "{1: 2, 3: 4}",
        ),
        (
            vec![
                ...
                atipicial_decompiler::OpCode::Push2.byte(),
                atipicial_decompiler::OpCode::Pack.byte(),
                atipicial_decompiler::OpCode::Unpack.byte(),
                atipicial_decompiler::OpCode::Drop.byte(),
                atipicial_decompiler::OpCode::Sub.byte(),
                atipicial_decompiler::OpCode::Ret.byte(),
            ],
            "return 1;",
        ),
...
fn structured_ir_preserves_setitem_mutation() {
    let aef = build_aef(&[0xC8, 0x11, 0x12, 0xD0, 0x40]);
    let ir = ir_for(&aef);
...
fn structured_ir_preserves_assert_and_message() {
    let assert_ir = ir_for(&build_aef(&[0x11, 0x39, 0x40]));
    let assert_message_ir = ir_for(&build_aef(&[0x11, 0x12, 0xE1, 0x40]));
...
fn structured_ir_routes_natural_endtry_through_finally() {
    ...
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
            "name": "NaturalFinally",
            "abi": { "methods": [{
    ...
    .unwrap();
    let mut decompilation = Decompiler::new()
        .decompile_bytes_with_manifest(&aef, Some(manifest), OutputFormat::All)
...
fn structured_ir_distinguishes_throw_abort_and_abort_message() {
    let throw_ir = ir_for(&build_aef(&[0x11, 0x3A]));
    let abort_ir = ir_for(&build_aef(&[0x38]));
    let abort_message_ir = ir_for(&build_aef(&[0x12, 0xE0]));
...
fn structured_ir_seeds_catch_exception_value() {
    ...
    let aef = build_aef(&[0x3B, 0x06, 0x00, 0x21, 0x3D, 0x05, 0x45, 0x3D, 0x02, 0x40]);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
            "name": "CatchState",
            "abi": { "methods": [{
...
fn ir_with_manifest_for(aef_path: &std::path::Path) -> String {
    let data = fs::read(aef_path).unwrap();
    ...
    let manifest = atipicial_decompiler::manifest::ContractManifest::from_json_str(&manifest)
...
fn ir_pipeline_recovers_a_switch_from_real_bytecode() {
    // Atipicial C# `switch` lowering (an equality cascade on one scrutinee):
...
fn ir_pipeline_recovers_an_if_from_a_real_artifact() {
    // LoopIf.aef has a conditional branch over the loop body; the IR structurer
...
fn ir_pipeline_is_well_formed_across_artifacts() {
    ...
    let root = repo_root();
    for aef_path in fs::read_dir(root.join("TestingArtifacts"))
        .unwrap()
        .flatten()
        .filter(|e| e.path().extension().and_then(|s| s.to_str()) == Some("aef"))
        .map(|e| e.path())
    {
        let data = match fs::read(&aef_path) {
            Ok(d) => d,
            Err(_) => continue,
        };
        let Ok(mut dec) =
...
fn ir_pipeline_renders_per_method_envelope_for_multimethod() {
    // MultiMethod.aef's manifest declares two methods (main@0, helper@6), but
    ...
    let root = repo_root();
    let aef_path = root.join("TestingArtifacts/edgecases/multi/MultiMethod.aef");
    let ir = ir_with_manifest_for(&aef_path);
    assert!(
...
fn ir_pipeline_loopif_envelope_preserves_while_loop() {
    // LoopIf.aef is a single-method artifact; the per-method envelope must keep
    // the structurer's `while` recovery intact inside the contract wrapper.
    let root = repo_root();
    let aef_path = root.join("TestingArtifacts/edgecases/LoopIf.aef");
    let ir = ir_with_manifest_for(&aef_path);
    assert!(
...
fn structured_ir_uses_manifest_parameter_names_in_signature_and_body() {
    ...
    let aef = build_aef(&[0x57, 0x00, 0x02, 0x79, 0x40]);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
            "name": "ParameterNames",
            "abi": {
...
fn structured_ir_disambiguates_parameter_from_generated_ssa_name() {
    ...
    let aef = build_aef(&[0x57, 0x01, 0x01, 0x11, 0x70, 0x68, 0x78, 0x9E, 0x40]);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
            "name": "NameCollision",
            "abi": {
...
fn structured_ir_keeps_recursive_private_return_contract_unknown() {
    let aef = build_aef(&[0x19, 0x34, 0x05, 0x40, 0x21, 0x21, 0x34, 0x00, 0x40]);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
...
fn structured_ir_seeds_inferred_entry_stack_arguments() {
    ...
    let aef = build_aef(&[0x12, 0x11, 0x34, 0x04, 0x40, 0x21, 0x9F, 0x40]);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
            "name": "EntryStackArgs",
            "abi": { "methods": [{
...
fn structured_ir_preserves_offsetless_entry_arity_for_recursive_call() {
    ...
    let aef = build_aef(&[0x78, 0x34, 0xFF, 0x40]);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
...
fn structured_ir_uses_manifest_argument_as_branch_condition() {
    ...
    let aef = build_aef(&[0x57, 0x00, 0x01, 0x78, 0x24, 0x04, 0x13, 0x40, 0x14, 0x40]);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
            "name": "ArgumentBranch",
            "abi": {
...
fn structured_ir_sanitizes_method_token_name() {
    let aef = build_aef_with_token(&[0x37, 0x00, 0x00, 0x40], "9-bad\nname", 0, false);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
...
fn structured_ir_uses_method_token_call_contract() {
    ...
    let aef = build_aef_with_token(&[0x19, 0x11, 0x37, 0x00, 0x00, 0x40], "notify", 1, false);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
            "name": "TokenCall",
            "abi": {
...
fn structured_ir_renders_known_syscall_value() {
    let aef = build_aef(&[0x11, 0x41, 0xF8, 0x27, 0xEC, 0x8C, 0x40]);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
            "name": "WitnessCheck",
            "abi": { "methods": [{
...
fn structured_ir_elides_known_syscall_temp_when_value_is_dropped() {
    let aef = build_aef(&[0x19, 0x11, 0x41, 0xF8, 0x27, 0xEC, 0x8C, 0x45, 0x40]);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
...
fn structured_ir_renders_known_syscall_void_as_statement() {
    let aef = build_aef(&[0x19, 0x11, 0x41, 0xCF, 0xE7, 0x47, 0x96, 0x40]);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
            "name": "RuntimeLog",
            "abi": { "methods": [{
...
fn structured_ir_pusha_uses_absolute_target_offset() {
    ...
    let aef = build_aef(&[0x10, 0x45, 0x0A, 0x05, 0x00, 0x00, 0x00, 0x40]);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
            "name": "PointerTarget",
            "abi": { "methods": [{
...
fn structured_ir_distinguishes_user_append_from_vm_append() {
    ...
    let aef = build_aef(&[0x11, 0x34, 0x03, 0x40, 0xC2, 0x4A, 0x11, 0xCF, 0x40]);
    let manifest = atipicial_decompiler::ContractManifest::from_json_str(
        r#"{
