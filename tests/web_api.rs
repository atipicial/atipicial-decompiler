...
use atipicial_decompiler::OutputFormat;
...
fn build_aef(script: &[u8]) -> Vec<u8> {
    ...
    let checksum = atipicial_decompiler::aef::AefParser::calculate_checksum(&data);
    data.extend_from_slice(&checksum.to_le_bytes());
    data
}

fn build_aef_with_unknown_opcode() -> Vec<u8> {
...
fn web_info_report_exposes_hashes_and_manifest_summary() {
    let report = atipicial_decompiler::web::info_report(&build_sample_aef(), Some(SAMPLE_MANIFEST))
        .expect("info report");
...
fn web_info_report_surfaces_manifest_extra_metadata() {
    ...
    "#;
    let report = atipicial_decompiler::web::info_report(&build_sample_aef(), Some(manifest_with_extra))
        .expect("info report");
...
fn web_disasm_report_surfaces_unknown_opcode_warnings() {
    let report = atipicial_decompiler::web::disasm_report(
        &build_aef_with_unknown_opcode(),
        atipicial_decompiler::web::WebDisasmOptions {
...
fn web_decompile_report_exposes_high_level_and_csharp_outputs() {
    let report = atipicial_decompiler::web::decompile_report(
        &build_sample_aef(),
        atipicial_decompiler::web::WebDecompileOptions {
    ...
    // AEF header fields are surfaced at the top level so JSON
    // consumers don't have to scrape the rendered text. The sample
    // AEF embeds `compiler = "test"` and an empty source.
    assert_eq!(
        value["compiler"].as_str().expect("compiler"),
        "test",
        "WebDecompileReport should surface the AEF compiler field at the top level",
    );
    assert!(
        value["source"].is_null(),
        "empty AEF source should serialize as null, not an empty string: {value}",
    );
}
