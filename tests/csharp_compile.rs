...
use atipicial_decompiler::{ContractManifest, Decompiler, OutputFormat};
...
fn framework_config() -> (PathBuf, String) {
    let framework = PathBuf::from(
        env::var_os("ATC_SMARTCONTRACT_FRAMEWORK_DLL")
            .expect("ATC_SMARTCONTRACT_FRAMEWORK_DLL is required for csharp_compile"),
    );
    assert!(
        framework.is_file(),
        "ATC_SMARTCONTRACT_FRAMEWORK_DLL is not a file: {}",
        framework.display()
    );
    let target_framework =
        env::var("ATC_CSHARP_TARGET_FRAMEWORK").unwrap_or_else(|_| "net8.0".to_string());
    assert!(
        ...
        "ATC_CSHARP_TARGET_FRAMEWORK must be a simple target moniker such as net8.0"
    );
...
fn write_project(project: &Path, framework: &Path, target_framework: &str) {
    fs::write(
        ...
        format!(
            "<Project Sdk=\"Microsoft.NET.Sdk\"><PropertyGroup><TargetFramework>{target_framework}</TargetFramework><Nullable>disable</Nullable><ImplicitUsings>disable</ImplicitUsings><ErrorLog>diagnostics.sarif,version=2.1</ErrorLog></PropertyGroup><ItemGroup><Reference Include=\"Atipicial.SmartContract.Framework\"><HintPath>{}</HintPath></Reference></ItemGroup></Project>",
...
fn read_manifest(aef_path: &Path) -> ContractManifest {
...
fn decompile_csharp(aef_path: &Path) -> String {
    let bytes = fs::read(aef_path).expect("read AEF");
    let manifest = read_manifest(aef_path);
    Decompiler::new()
        .with_trace_comments(false)
        .with_typed_declarations(true)
        .decompile_bytes_with_manifest(&bytes, Some(manifest), OutputFormat::All)
        .expect("decompile AEF")
        .csharp
        .expect("C# output")
}
...
fn collect_aef_files(path: &Path, files: &mut Vec<PathBuf>) {
    for entry in fs::read_dir(path).expect("read corpus directory") {
        let path = entry.expect("read corpus entry").path();
        if path.is_dir() {
            collect_aef_files(&path, files);
        } else if path.extension().is_some_and(|extension| extension == "aef") {
            files.push(path);
        }
    }
}
...
fn representative_generated_csharp_compiles_with_roslyn() {
    if env::var_os("ATC_SMARTCONTRACT_FRAMEWORK_DLL").is_none() {
        eprintln!(
            "ATC_SMARTCONTRACT_FRAMEWORK_DLL is unset; skipping representative C# corpus gate"
        );
        return;
    }
    ...
    let cases = [
        "TestingArtifacts/edgecases/LoopIf.aef",
        "TestingArtifacts/edgecases/multi/MultiMethod.aef",
        "TestingArtifacts/edgecases/events/Events.aef",
        "TestingArtifacts/edgecases/permissions/Permissions.aef",
        "TestingArtifacts/edgecases/MethodToken.aef",
    ];

    for case in cases {
        let aef_path = root.join(case);
        let source = decompile_csharp(&aef_path);
...
fn pinned_corpus_generated_csharp_compiles_with_roslyn() {
    let Some(corpus) = env::var_os("ATC_CSHARP_CORPUS_DIR").map(PathBuf::from) else {
        eprintln!("ATC_CSHARP_CORPUS_DIR is unset; skipping full C# corpus gate");
        return;
    };
    let (framework, target_framework) = framework_config();
    ...
    assert_eq!(
        ...
        "C# corpus is not the pinned atipicial-devpack-dotnet v3.10.0 revision"
    );

    let mut aef_files = Vec::new();
    collect_aef_files(&corpus, &mut aef_files);
    aef_files.sort();
    assert_eq!(aef_files.len(), 103, "pinned C# corpus count drift");
    for aef_path in &aef_files {
        let _ = read_manifest(aef_path);
    }

    let mut contract_names = BTreeSet::new();
    let generated_sources = aef_files
        .iter()
        .map(|aef_path| {
            let contract_name = aef_path
                .file_stem()
                .and_then(|stem| stem.to_str())
            ...
            (contract_name, decompile_csharp(aef_path))
        })
        .collect::<Vec<_>>();
    ...
    let passed = aef_files.len() - contract_errors.len();
