...
//
// AtipicialCoin.cs file belongs to the atipicial project and is free
...
{
    /// <summary>
    /// Represents the ATC token in the ATC system.
    /// </summary>
    public sealed class AtipicialCoin : FungibleToken<AtipicialCoin.AtipicialAccountState>
    {
        public override string Symbol => "ATC";
        public override byte Decimals => 0;

        /// <summary>
        /// Indicates the total amount of ATC.
        /// </summary>
        public BigInteger TotalAmount { get; }

        /// <summary>
        /// Indicates the effective voting turnout in ATC. The voted candidates will only be effective when the voting turnout exceeds this value.
        /// </summary>
        ...
        internal override void OnBalanceChanging(ApplicationEngine engine, UInt160 account, AtipicialAccountState state, BigInteger amount)
        {
        ...
        protected override void OnManifestCompose(IsHardforkEnabledDelegate hfChecker, uint blockHeight, ContractManifest manifest)
        {
            ...
            {
                manifest.SupportedStandards = ["AEP-17", "AEP-27"];
            }
            else
            {
                manifest.SupportedStandards = ["AEP-17"];
            }
        }

        private GasDistribution? DistributeGas(ApplicationEngine engine, UInt160 account, AtipicialAccountState state)
        {
        ...
        private BigInteger CalculateBonus(DataCache snapshot, AtipicialAccountState state, uint end)
        {
            ...
            (var atipicialHolderReward, var voteReward) = CalculateReward(snapshot, state, end);
            return atipicialHolderReward + voteReward;
        }

        private (BigInteger atipicialHold, BigInteger voteReward) CalculateReward(DataCache snapshot, AtipicialAccountState state, uint end)
        {
            ...
            return (state.Balance * sumGasPerBlock * AtipicialHolderRewardRatio / 100 / TotalAmount, voteReward);
        }
        ...
        public BigInteger UnclaimedGas(ApplicationEngine engine, UInt160 account, uint end)
        {
            ...
            AtipicialAccountState state = storage.GetInteroperable<AtipicialAccountState>();
        ...
        private async ContractTask OnAEP17Payment(ApplicationEngine engine, UInt160 from, BigInteger amount, StackItem data)
        {
        ...
        {
            AtipicialAccountState? stateAccount = engine.SnapshotCache.GetAndChange(CreateStorageKey(Prefix_Account, account))?.GetInteroperable<AtipicialAccountState>();
        ...
        [ContractMethod(CpuFee = 1 << 15, RequiredCallFlags = CallFlags.ReadStates)]
        public AtipicialAccountState? GetAccountState(IReadOnlyStore snapshot, UInt160 account)
        {
            var key = CreateStorageKey(Prefix_Account, account);
            return snapshot.TryGet(key, out var item) ? item.GetInteroperableClone<AtipicialAccountState>() : null;
        }
        ...
        /// <summary>
        /// Represents the account state of <see cref="AtipicialCoin"/>.
        /// </summary>
        public class AtipicialAccountState : AccountState
        {
            /// <summary>
