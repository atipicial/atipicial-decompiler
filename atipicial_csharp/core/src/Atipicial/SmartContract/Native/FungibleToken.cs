...
//
// FungibleToken.cs file belongs to the atipicial project and is free
...
{
    /// <summary>
    /// The base class of all native tokens that are compatible with AEP-17.
    /// </summary>
        ...
        where TState : AccountState, new()
    {
        ...
        {
            manifest.SupportedStandards = new[] { "AEP-17" };
        }
        ...
        private protected virtual async ContractTask PostTransferAsync(ApplicationEngine engine, UInt160? from, UInt160? to, BigInteger amount, StackItem data, bool callOnPayment)
        {
            ...
            // Call onAEP17Payment method

            await engine.CallFromNativeContractAsync(Hash, to, "onAEP17Payment", from?.ToArray() ?? StackItem.Null, amount, data);
        }
    }
}
