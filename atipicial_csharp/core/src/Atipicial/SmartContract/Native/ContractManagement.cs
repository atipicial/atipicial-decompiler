...
//
// ContractManagement.cs file belongs to the atipicial project and is free
...
using Array = Atipicial.VM.Types.Array;
...
namespace Atipicial.SmartContract.Native
{
    ...
    public sealed class ContractManagement : NativeContract
    {
            ...
            foreach (NativeContract contract in Contracts)
            {
                if (contract.IsInitializeBlock(engine.ProtocolSettings, engine.PersistingBlock!.Index, out var hfs))
                {
                    ...
                    else
                    {
                        ...
                        // Modify aef and manifest
                        oldContract.Aef = contractState.Aef;
        ...
        /// </summary>
        /// <param name="engine">The engine used to write data.</param>
        /// <param name="aefFile">The AEF file of the contract.</param>
        ...
        private ContractTask<ContractState> Deploy(ApplicationEngine engine, byte[] aefFile, byte[] manifest)
        {
            return Deploy(engine, aefFile, manifest, StackItem.Null);
        }
        ...
        /// <returns>The deployed contract.</returns>
        [ContractMethod(RequiredCallFlags = CallFlags.States | CallFlags.AllowNotify)]
        private async ContractTask<ContractState> Deploy(ApplicationEngine engine, byte[] aefFile, byte[] manifest, StackItem data)
        {
            ...
            if (aefFile.Length == 0)
                throw new ArgumentException($"AEF file length cannot be zero.");
            if (manifest.Length == 0)
                throw new ArgumentException($"Manifest length cannot be zero.");

            // In the unit of picoGAS, 1 picoGAS = 1e-12 GAS
            engine.AddFee(BigInteger.Max(engine.StoragePrice * (aefFile.Length + manifest.Length),
                GetMinimumDeploymentFee(engine.SnapshotCache))
                * ApplicationEngine.FeeFactor);

            AefFile aef = aefFile.AsSerializable<AefFile>();
            ContractManifest parsedManifest = ContractManifest.Parse(manifest);
            Helper.Check(new Script(aef.Script, engine.IsHardforkEnabled(Hardfork.HF_Basilisk)), parsedManifest.Abi);
            UInt160 hash = Helper.GetContractHash(tx.Sender, aef.CheckSum, parsedManifest.Name);
        ...
        [ContractMethod(RequiredCallFlags = CallFlags.States | CallFlags.AllowNotify)]
        private ContractTask Update(ApplicationEngine engine, byte[]? aefFile, byte[]? manifest)
        {
            return Update(engine, aefFile, manifest, StackItem.Null);
        }
        ...
        /// <returns>The updated contract.</returns>
        [ContractMethod(RequiredCallFlags = CallFlags.States | CallFlags.AllowNotify)]
        private ContractTask Update(ApplicationEngine engine, byte[]? aefFile, byte[]? manifest, StackItem data)
        {
            // Require CallFlags.All flag for post-Aspidochelone transactions, ref. #2653, #2673.
            if (engine.IsHardforkEnabled(Hardfork.HF_Aspidochelone))
            {
            ...
            if (aefFile is null && manifest is null)
                throw new ArgumentException("AEF file and manifest cannot both be null.");

            engine.AddFee(engine.StoragePrice * ApplicationEngine.FeeFactor * ((aefFile?.Length ?? 0) + (manifest?.Length ?? 0)));
            ...
            if (aefFile != null)
            {
                ...
                // Update aef
                contract.Aef = aefFile.AsSerializable<AefFile>();
            }
            ...
            Helper.Check(new Script(contract.Aef.Script, engine.IsHardforkEnabled(Hardfork.HF_Basilisk)), contract.Manifest.Abi);
