...
//
// PolicyContract.cs file belongs to the atipicial project and is free
...
namespace Atipicial.SmartContract.Native
{
    ...
    public sealed class PolicyContract : NativeContract
    {
        ...
        internal async ContractTask<bool> BlockAccountInternal(ApplicationEngine engine, UInt160 account)
        {
            ...
            if (engine.IsHardforkEnabled(Hardfork.HF_Faun))
                await ATC.VoteInternal(engine, account, null);
        ...
        internal async ContractTask<bool> RecoverFund(ApplicationEngine engine, UInt160 account, UInt160 token)
        {
            ...
            // Validate contract implements AEP-17 standard
            if (!contract.Manifest.SupportedStandards.Contains("AEP-17"))
                throw new InvalidOperationException($"Contract {token} does not implement AEP-17 standard.");
